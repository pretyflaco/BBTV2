...
export default function BatchPayments({
  ...
  useEffect(() => {
    if (currentStep === STEPS.VALIDATING || currentStep === STEPS.EXECUTING) {
      ...
      const recipientCount = currentStep === STEPS.EXECUTING
        ? progress.total
  ...
  const handleValidate = useCallback(async () => {
    ...
    try {
      ...
      const totalAmount = data.summary.totalAmountSats;

      // Estimate fees (0 for intra-ledger, ~0.3% for external)
      const blinkCount = data.summary.byType.BLINK || 0;
...
machankura@8333.mobi,2000,SATS,Payment to Machankura user`;
  ...
  const handleDownloadResults = useCallback(() => {
    ...
    const rows = executionResults.results.map(r =>
  ...
  if (currentStep === STEPS.VALIDATING) {
    ...
    return (
      <div className="h-full flex flex-col bg-white dark:bg-black p-4 items-center justify-center" style={{ fontFamily: "'Source Sans Pro', sans-serif" }}>
  ...
  if (currentStep === STEPS.CONFIRM && feeSummary) {
    const insufficientBalance = balanceSats !== undefined && balanceSats < feeSummary.grandTotalSats;
        ...
        <div className="flex-1 p-4">
          <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-6">
            ...
            <div className="space-y-3">
              ...
              {/* Network Fees */}
              <div className="flex justify-between">
                <span className="text-gray-600 dark:text-gray-400">Network Fees (est.)</span>
                <span className="font-medium text-gray-900 dark:text-white">
              ...
              {balanceSats !== undefined && (
                <>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className={`font-medium ${insufficientBalance ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
                      {balanceSats.toLocaleString()} sats
                    </span>
                  </div>
                  {!insufficientBalance && (
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500 dark:text-gray-500">After Payment</span>
                      <span className="text-gray-600 dark:text-gray-400">
            ...
            {insufficientBalance && (
              <div className="mt-4 p-3 bg-red-100 dark:bg-red-900/30 border border-red-300 dark:border-red-700 rounded-lg">
                <p className="text-sm text-red-700 dark:text-red-300">
                  Insufficient balance. You need {(feeSummary.grandTotalSats - balanceSats).toLocaleString()} more sats.
                </p>
              </div>
            )}
            ...
            <p className="mt-4 text-xs text-gray-500 dark:text-gray-400 text-center">
              Fees are estimated. Actual fees may vary based on Lightning network conditions.
            </p>
          </div>
        </div>
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="text-center mb-4">
            <p className="text-gray-500 dark:text-gray-400 animate-pulse">
              ...
              {displayProgress >= 30 && displayProgress < 60 && "Processing transactions..."}
          ...
          <div className="flex justify-center gap-2 mt-4">
            ...
            <span className="w-2 h-2 bg-teal-300 rounded-full animate-bounce" style={{ animationDelay: '300ms' }}></span>
          </div>
