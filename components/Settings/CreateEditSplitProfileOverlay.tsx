...
interface CreateEditSplitProfileOverlayProps {
  ...
  setEditingSplitProfile: (profile: SplitProfile | null) => void
...
}: CreateEditSplitProfileOverlayProps) {
          ...
          <div className="space-y-4">
            ...
            <div className={`rounded-lg p-4 ${darkMode ? "bg-gray-900" : "bg-gray-50"}`}>
              ...
              <input
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setNewSplitProfileLabel(e.target.value)
                }
            ...
            <div className={`rounded-lg p-4 ${darkMode ? "bg-gray-900" : "bg-gray-50"}`}>
                ...
                <div className="mb-3 space-y-2">
                  {newSplitProfileRecipients.map((recipient, index) => (
                    ...
                    >
                      <span className="text-sm text-green-700 dark:text-green-400 flex-1">
                        {recipient.type === "npub_cash"
                          ? recipient.username
                          : `${recipient.username}@${lnAddressDomain}`}
                      </span>
                      {useCustomWeights && (
                        <div className="flex items-center mx-2">
                          <input
                            ...
                            max="99"
                            value={Math.round(
                              recipient.weight || 100 / newSplitProfileRecipients.length,
                            )}
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                              ...
                              ;(
                                setNewSplitProfileRecipients as (
                                  fn: (prev: SplitRecipient[]) => SplitRecipient[],
                                ) => void
                              )((prev: SplitRecipient[]) => {
                                // Mark this recipient as locked (manually edited)
                                const updated = prev.map((r, i) =>
                                  i === index
                                    ? { ...r, weight: newWeight, locked: true }
                                    : r,
                                )
                                ...
                                const unlockedRecipients = updated.filter(
                                  (r) => !r.locked,
                                )
                                ...
                                if (unlockedRecipients.length > 0) {
                                  ...
                                  const weightPerUnlocked =
                                    remainingWeight / unlockedRecipients.length
                            ...
                            <button
                              onClick={() => {
                                ...
                                ;(
                                  ...
                                  const unlockedRecips = updated.filter((r) => !r.locked)
                                  if (unlockedRecips.length > 0) {
                                    const remainingWeight = Math.max(0, 100 - lockedSum)
                                    const weightPerUnlocked =
                                      remainingWeight / unlockedRecips.length
                  ...
                  {useCustomWeights ? (
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      Split will be divided evenly (
                      {(100 / newSplitProfileRecipients.length).toFixed(1)}% each)
                    </p>
                  )}
                </div>
              )}
              ...
              {recipientValidation.status === "success" && newRecipientInput && (
                ...
                >
                  Add{" "}
                  {recipientValidation.type === "npub_cash"
                    ? recipientValidation.address
                    : `${newRecipientInput}@${lnAddressDomain}`}
                </button>
              )}
              {newSplitProfileRecipients.length === 0 && (
            ...
            <button
              onClick={async () => {
                ...
                if (useCustomWeights && newSplitProfileRecipients.length > 1) {
                  ...
                  if (Math.abs(totalWeight - 100) > 0.01) {
                    setSplitProfileError(
                      `Total split weights must equal 100% (currently ${Math.round(totalWeight)}%)`,
                    )
                    return
                  }
              ...
              disabled={
                !newSplitProfileLabel.trim() || newSplitProfileRecipients.length === 0
              }
