...
export default function PreferencesSection() {
  ...
  const [showSatsFirst, setShowSatsFirst] = useState<boolean>(
    preferences?.showSatsFirst ?? false,
  )
  ...
  const allCurrencies: CurrencyItem[] = currencies
    ? (getAllCurrencies() as unknown as CurrencyItem[])
    : []
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`rounded-lg p-4 ${darkMode ? "bg-gray-900" : "bg-gray-50"}`}>
        <h4
          className={`font-medium text-sm mb-2 ${darkMode ? "text-white" : "text-gray-900"}`}
        >
          Theme
        </h4>
      ...
      <div className={`rounded-lg p-4 ${darkMode ? "bg-gray-900" : "bg-gray-50"}`}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Toggle
              value={showSatsFirst}
              onChange={() => setShowSatsFirst(!showSatsFirst)}
            />
      ...
      {/* App Info */}
      <div
        className={`pt-4 border-t text-center ${darkMode ? "border-gray-700" : "border-gray-200"}`}
      >
