...
interface LabelSelectorProps {
  currentLabel: TransactionLabelDef
  onSelectLabel: (label: TransactionLabelDef) => void
  darkMode: boolean
  isSyncing: boolean
}
...
interface TransactionDetailProps {
  ...
  onClose: () => void
  darkMode?: boolean
  onLabelChange?: (txId: string, label: TransactionLabelDef) => void
}
...
const TRANSACTION_LABELS: TransactionLabelDef[] = [
  {
    id: "none",
    name: "No Label",
    color: "gray",
...
let labelsCache: LabelsMap | null = null
...
const loadLocalLabels = (): LabelsMap => {
  try {
    const stored = localStorage.getItem(TX_LABELS_STORAGE_KEY)
    return stored ? (JSON.parse(stored) as LabelsMap) : {}
  } catch (err: unknown) {
...
const saveLocalLabels = (labels: LabelsMap): void => {
  try {
...
const loadServerLabels = async (): Promise<LabelsMap> => {
  try {
    ...
    if (response.ok) {
      ...
      if (data.transactionLabels) {
        ...
        const mergedLabels: LabelsMap = { ...localLabels, ...data.transactionLabels }
...
const saveServerLabels = async (labels: LabelsMap): Promise<void> => {
  try {
    const response = await fetch("/api/user/sync", {
      method: "PATCH",
...
export const getAllTransactionLabels = (): LabelsMap => {
  if (labelsCache === null) {
...
export const getTransactionLabel = (txId: string): TransactionLabelDef => {
...
export const initTransactionLabels = async (): Promise<LabelsMap> => {
...
const getTransactionType = (settlementVia?: SettlementVia): string => {
...
// Color lookup helper for label dots
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div
            ...
            style={{
              backgroundColor: getLabelColorHex(currentLabel.color),
            }}
          ></div>
          <span
            className={`text-sm font-medium ${darkMode ? currentLabel.textDark : currentLabel.textLight}`}
          >
            {currentLabel.name}
          </span>
        </div>
          ...
          {TRANSACTION_LABELS.map((label) => (
            ...
            >
              ...
              <div
                ...
                style={{
                  borderColor: getLabelColorHex(label.color),
                  backgroundColor:
                    label.id !== "none"
                      ? `${getLabelColorHex(label.color)}20`
                      : "transparent",
                }}
              >
                {currentLabel.id === label.id && (
                  <svg
                    ...
                    style={{ color: getLabelColorHex(label.color) }}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
              ...
              <span
                className={`text-sm font-medium ${darkMode ? label.textDark : label.textLight}`}
              >
                {label.name}
              </span>
            </button>
...
export default function TransactionDetail({
  ...
  const [currentLabel, setCurrentLabel] = useState<TransactionLabelDef>(
  ...
  const handleSelectLabel = async (label: TransactionLabelDef): Promise<void> => {
    setCurrentLabel(label)
    setIsSyncing(true)
  ...
  // Get payment hash
  const getPaymentHash = (): string | null => {
    if (initiationVia?.__typename === "InitiationViaLn") {
      return initiationVia.paymentHash || null
    }
    return null
  }

  // Get preimage
  const getPreimage = (): string | null => {
    if (
      settlementVia?.__typename === "SettlementViaLn" ||
      settlementVia?.__typename === "SettlementViaIntraLedger"
    ) {
      return (settlementVia.preImage as string) || null
    }
    return null
  }

  // Get on-chain transaction hash
  const getOnChainHash = (): string | null => {
    if (settlementVia?.__typename === "SettlementViaOnChain") {
      return settlementVia.transactionHash || null
    }
    return null
  }

  // Get on-chain address
  const getOnChainAddress = (): string | null => {
    if (initiationVia?.__typename === "InitiationViaOnChain") {
      return initiationVia.address || null
    }
    return null
  }
  ...
  const handleDownloadReceipt = async (): Promise<void> => {
    ...
    try {
      ...
      link.href = url
      link.download = `blink-receipt-${(transactionId as string).slice(0, 8)}.pdf`
  ...
  return (
    <div
      className={`fixed inset-0 z-50 overflow-y-auto ${darkMode ? "bg-black" : "bg-white"}`}
    >
      <div className="min-h-screen">
        {/* Header - Sticky */}
        <div
          className={`sticky top-0 z-10 shadow ${darkMode ? "bg-blink-dark shadow-black" : "bg-gray-50"}`}
        >
        ...
        <div className={`py-8 px-4 ${darkMode ? "bg-gray-900" : "bg-gray-50"}`}>
          <div className="max-w-md mx-auto text-center">
            ...
            {secondaryAmount && (
              <p
                className={`text-lg mt-1 ${darkMode ? "text-gray-400" : "text-gray-500"}`}
              >
                {secondaryAmount}
              </p>
            )}
        ...
        <div className="max-w-md mx-auto px-4 py-6">
          ...
          <DetailRow
            ...
            value={
              <div className="flex items-center gap-2">
                ...
                <span>
                  {settlementCurrency === "BTC" ? "BTC Account" : "USD Account"}
                </span>
              </div>
            }
            darkMode={darkMode}
          ...
          {/* Date */}
          <DetailRow label="Date" value={date} darkMode={darkMode} />

          {/* Fee (only for sent transactions) */}
          {!isReceive && fee && <DetailRow label="Fee" value={fee} darkMode={darkMode} />}
          ...
          {/* Type */}
          <DetailRow label="Type" value={txType} darkMode={darkMode} />
