 ...
 */
function getStatusInfo(status, darkMode) {
  switch (status) {
...
export default function BoltcardCard({
  ...
  const CurrencyIcon = card.walletCurrency === 'USD' ? (
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs px-1.5 py-0.5 rounded ${statusInfo.bgColor} ${statusInfo.textColor}`}>
                  {statusInfo.label}
                </span>
              </div>
            </div>
          ...
          {/* Arrow */}
          <svg className={`w-5 h-5 flex-shrink-0 ${darkMode ? 'text-gray-600' : 'text-gray-400'}`} 
      ...
      {(card.maxTxAmount || card.dailyLimit) && (
        <div className={`mb-4 p-2 rounded-lg ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
          <div className="flex gap-4 text-xs">
            {card.maxTxAmount && (
              <div>
                <span className={darkMode ? 'text-gray-500' : 'text-gray-400'}>Max/tx: </span>
            ...
            {card.dailyLimit && (
              <div>
                <span className={darkMode ? 'text-gray-500' : 'text-gray-400'}>Daily: </span>
                <span className={darkMode ? 'text-gray-300' : 'text-gray-600'}>
