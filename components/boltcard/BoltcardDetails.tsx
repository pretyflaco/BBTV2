...
/** Wipe keys data from the API */
interface WipeKeysData {
...
/** Tab button props (inner component) */
...
/** Props for the BoltcardDetails component */
export interface BoltcardDetailsProps {
  card: BoltcardRecord
  onClose: () => void
  onUpdate: (cardId: string, updates: CardUpdatePayload) => Promise<CardActionResult>
  onDisable: (cardId: string) => Promise<CardActionResult>
  onEnable: (cardId: string) => Promise<CardActionResult>
  onWipe: (cardId: string) => Promise<CardActionResult>
  onResetDaily: (cardId: string) => Promise<CardActionResult>
  onFund: (card: BoltcardRecord) => void
  onFundCard: (cardId: string, amount: number, mode?: string) => Promise<FundResult>
  fetchDetails: (cardId: string, includeTopUpQR?: boolean) => Promise<FetchDetailsResult>
 ...
 */
function formatDate(dateString: string | number | null | undefined): string {
 ...
 */
function getTransactionTypeInfo(type: string): TransactionTypeInfo {
  switch (type) {
    case "topup":
 ...
 */
function generateResetDeeplink(cardId: string): string {
...
export default function BoltcardDetails({
  ...
  exchangeRate = null,
  bitcoinFormat = "sats",
}: BoltcardDetailsProps) {
  const { darkMode } = useTheme()
  const [activeTab, setActiveTab] = useState<TabValue>(Tabs.DETAILS)
  const [loading, setLoading] = useState(false)
  const [transactions, setTransactions] = useState<BoltcardTransaction[]>([])
  const [topUpQR, setTopUpQR] = useState<TopUpQRData | null>(null)

  // Local card state to enable immediate UI updates after funding
  const [card, setCard] = useState<BoltcardRecord>(initialCard)
    ...
    const loadDetails = async () => {
      if (fetchDetails) {
        ...
        if (result.success) {
          ...
          setTopUpQR(result.topUpQR ?? null)
        }
        setLoading(false)
      }
    }
  ...
  const handleWipe = async () => {
    if (
      !window.confirm("Are you sure you want to wipe this card? This cannot be undone.")
    ) {
      return
    }
  ...
  const handleShowResetModal = async () => {
    ...
    try {
      ...
      const data = (await response.json()) as WipeKeysData
      setWipeKeys(data)
    } catch (err: unknown) {
      console.error("Failed to fetch wipe keys:", err)
      setWipeKeysError((err as Error).message)
    } finally {
   ...
   */
  const handleFundFromTopUp = async (
    cardId: string,
    newBalance: number,
    mode: string,
  ): Promise<FundResult> => {
    ...
    try {
      ...
      return {
        success: false,
        error: (err as Error).message || "Failed to update balance",
      }
    } finally {
      setLoading(false)
    }
  ...
  const renderResetModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-[60] bg-black/70 flex items-center justify-center p-4">
        <div
          className={`w-full max-w-sm max-h-[90vh] rounded-xl shadow-2xl overflow-hidden flex flex-col ${
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {wipeKeysError && (
              ...
              >
                <h5
                  className={`text-sm font-medium mb-1 ${darkMode ? "text-red-400" : "text-red-700"}`}
                >
                ...
                {!keysRevealed ? (
                  <div className="space-y-4">
                    ...
                    >
                      ...
                      >
                        The keys you are about to view are sensitive cryptographic
                        secrets.
                        <strong>
                          {" "}
                          Anyone with access to these keys can reset your card.
                        </strong>
                      </p>
                      <ul
                  ...
                  <>
                    ...
                    >
                      <div
                        className={`px-3 py-2 ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}
                      >
                        <h5
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            {activeTab === Tabs.DETAILS && (
              <div className="space-y-4">
                ...
                {walletBalance !== undefined && (
                  ...
                  >
                    ...
                    {walletBalance < (card.balance || 0) && (
                      ...
                      >
                        Card allocation exceeds wallet funds. Card can only spend
                ...
                {/* Card Info */}
                <div
                  className={`rounded-lg p-3 space-y-2 ${darkMode ? "bg-gray-900" : "bg-gray-50"}`}
                >
                  ...
                  {card.cardUid ? (
                    ...
                    card.cardIdHash && (
                      <div className="flex justify-between text-sm">
                        ...
                        <span
                          className={`font-mono text-xs ${darkMode ? "text-white" : "text-gray-900"}`}
                        >
            ...
            {activeTab === Tabs.HISTORY && (
              <div className="space-y-2">
                {loading ? (
                  <div className="text-center py-8">
                    <div
                      className={`animate-spin w-6 h-6 border-2 border-blink-accent border-t-transparent rounded-full mx-auto`}
                    />
                  </div>
            ...
            {activeTab === Tabs.SETTINGS && (
              <div className="space-y-4">
                {/* Edit Form */}
                <div
                  className={`rounded-lg p-3 ${darkMode ? "bg-gray-900" : "bg-gray-50"}`}
                >
                  <h4
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      >
                        Max per transaction (
                        {card.walletCurrency === "USD" ? "USD" : "sats"})
                      </label>
                      <input
                        type="number"
                        value={editMaxTx}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          setEditMaxTx(e.target.value)
                        }
                ...
                >
                  <h4
                    className={`text-sm font-medium mb-2 ${darkMode ? "text-orange-400" : "text-orange-700"}`}
                  >
                    Reset Card
                  </h4>
                  <p
                ...
                >
                  <h4
                    className={`text-sm font-medium mb-2 ${darkMode ? "text-red-400" : "text-red-700"}`}
                  >
                    Danger Zone
                  </h4>
                  <p
