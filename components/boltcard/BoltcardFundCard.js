 ...
 */
const QUICK_AMOUNTS_BTC = [1000, 5000, 10000, 50000, 100000];
const QUICK_AMOUNTS_USD = [100, 500, 1000, 2500, 5000]; // in cents
...
export default function BoltcardFundCard({
  card,
  walletBalance, // Available balance in the Sending Wallet (sats for BTC, cents for USD)
  onFund,
  onClose,
  loading = false,
    ...
    <div className="fixed inset-0 z-[60] bg-black/70 flex items-center justify-center p-4">
      <div className={`w-full max-w-sm rounded-xl shadow-2xl overflow-hidden ${
        ...
        <div className="p-4 space-y-4">
          ...
          <div className={`p-3 rounded-lg ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`text-xs px-2 py-0.5 rounded ${
                darkMode ? 'bg-blink-accent/20 text-blink-accent' : 'bg-blink-accent/10 text-blink-accent'
              }`}>
                {card.walletCurrency}
              </span>
            </div>
          ...
          {/* New Balance Preview */}
          {parsedAmount > 0 && (
            <div className={`p-3 rounded-lg ${darkMode ? 'bg-green-900/10' : 'bg-green-50'}`}>
              <div className="flex justify-between items-baseline">
