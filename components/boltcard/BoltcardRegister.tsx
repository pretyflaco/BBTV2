...
/** Voucher wallet shape (from parent) */
interface VoucherWallet {
  apiKey: string
  [key: string]: unknown
}

/** Props for the BoltcardRegister component */
export interface BoltcardRegisterProps {
  onRegister: (params: RegisterCardParams) => Promise<RegisterResult>
  onCancel: () => void
  loading?: boolean
...
export default function BoltcardRegister({
  ...
  voucherWalletBtcId,
  voucherWalletUsdId,
}: BoltcardRegisterProps) {
  const { darkMode } = useTheme()

  // Form state
  const [step, setStep] = useState<StepValue>(Steps.WALLET_SETTINGS)
  const [name, setName] = useState("")
  const [walletCurrency, setWalletCurrency] = useState<CardCurrencyValue>(
    CardCurrency.BTC,
  )
  ...
  const [registrationResult, setRegistrationResult] = useState<RegisterResult | null>(
    null,
  )
  ...
  const handleRegister = async () => {
    ...
    const walletId =
      walletCurrency === CardCurrency.USD ? voucherWalletUsdId : voucherWalletBtcId

    if (!walletId) {
      setError(
        `No ${walletCurrency} wallet available. Please configure a Sending Wallet with a ${walletCurrency} wallet.`,
      )
      setIsRegistering(false)
      return
    }
    ...
    let parsedMaxTx: number | null = null
      ...
      <div className="space-y-4">
        <div className="text-center">
          <div className="w-12 h-12 mx-auto mb-3 rounded-full bg-blink-accent/20 flex items-center justify-center">
            <svg
              className="w-6 h-6 text-blink-accent"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        {isDeeplinkFlow && expiresIn !== null && (
          ...
          >
            <span className="text-sm">
              Expires in{" "}
              <span className="font-mono font-medium">{formatCountdown(expiresIn)}</span>
            </span>
          </div>
        )}
        ...
        {qrPayload && (
          <div className={`rounded-lg ${darkMode ? "bg-gray-800" : "bg-gray-100"}`}>
            ...
            {isMobile ? (
              ...
              >
                <span className="text-sm font-medium">Or scan with another device</span>
                <svg
            ...
            {(showQR || !isMobile) && (
              <div
                className={`p-4 ${isMobile ? "border-t " + (darkMode ? "border-gray-700" : "border-gray-200") : ""}`}
              >
        ...
        {pendingReg && (
          <div
            className={`p-3 rounded-lg text-sm ${darkMode ? "bg-gray-900" : "bg-gray-50"}`}
          >
            {pendingReg.name && (
        ...
        >
          <h5
            className={`text-sm font-medium mb-2 ${darkMode ? "text-blue-400" : "text-blue-700"}`}
          >
            Programming Steps:
          </h5>
          <ol
