...
/** Voucher wallet object shape from parent */
...
/** Props for the BoltcardSection component */
export interface BoltcardSectionProps {
  voucherWallet?: VoucherWallet | null
  voucherWalletBtcId?: string | null
  voucherWalletUsdId?: string | null
  voucherWalletBtcBalance?: number
...
export default function BoltcardSection({
  voucherWallet = null,
  voucherWalletBtcId = null,
  voucherWalletUsdId = null,
  voucherWalletBtcBalance = 0,
  voucherWalletUsdBalance = 0,
  exchangeRate = null,
  bitcoinFormat = "sats",
}: BoltcardSectionProps) {
  ...
  const [fundingCard, setFundingCard] = useState<BoltcardRecord | null>(null)

  /**
   * Handle card registration
   */
  const handleRegister = async (
    cardData: RegisterCardParams,
   ...
   */
  const handleViewDetails = (card: BoltcardRecord) => {
   ...
   * Handle disable card
   */
  const handleDisable = async (cardId: string): Promise<CardActionResult> => {
    ...
    if (result.success && selectedCard?.id === cardId) {
      ...
      setSelectedCard((prev) =>
        prev ? { ...prev, status: result.card?.status ?? prev.status } : prev,
      )
    }
    return result
  }
   ...
   * Handle enable card
   */
  const handleEnable = async (cardId: string): Promise<CardActionResult> => {
  ...
  const handleUpdate = async (
    ...
    if (result.success && selectedCard?.id === cardId) {
      setSelectedCard((prev) => (prev ? { ...prev, ...result.card } : prev))
    }
    return result
  }
   ...
   * Handle wipe card
   */
  const handleWipe = async (cardId: string): Promise<CardActionResult> => {
   ...
   */
  const handleResetDaily = async (cardId: string): Promise<CardActionResult> => {
    const result = await resetDailySpent(cardId)
    if (result.success && selectedCard?.id === cardId) {
      setSelectedCard((prev) => (prev ? { ...prev, dailySpent: 0 } : prev))
    }
    return result
  }
   ...
   * Handle fund card
   */
  const handleFund = async (cardId: string, amount: number): Promise<FundResult> => {
    const result = await fundCard(cardId, amount)
    if (result.success) {
      // Update selected card balance if it's the same card
      if (selectedCard?.id === cardId) {
        setSelectedCard((prev) =>
          prev ? { ...prev, balance: result.card?.balance ?? prev.balance } : prev,
        )
      }
    }
    return result
   ...
   */
  const getWalletBalanceForCard = (card: BoltcardRecord): number => {
      ...
      <div className="space-y-4">
        ...
        >
          <div className="flex items-start gap-3">
            <svg
              className={`w-5 h-5 flex-shrink-0 ${darkMode ? "text-yellow-500" : "text-yellow-600"}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <div>
              ...
              <p
                className={`text-xs mt-1 ${darkMode ? "text-yellow-300" : "text-yellow-600"}`}
              >
                Configure a Sending Wallet in Settings first. Boltcards use this wallet to
                fund withdrawals.
              </p>
            </div>
          </div>
    ...
    <div className="space-y-4">
      ...
      {!showRegister && !loading && cards.length === 0 && (
        ...
        >
          ...
          >
            <svg
              className={`w-6 h-6 ${darkMode ? "text-gray-600" : "text-gray-400"}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
      ...
      {selectedCard && (
        <BoltcardDetails
          ...
          onFund={(card: BoltcardRecord) => setFundingCard(card)}
          onFundCard={fundCard}
      ...
      {/* Recovery Modal */}
      {showRecovery && <BoltcardRecovery onClose={() => setShowRecovery(false)} />}
    </div>
  )
}
