...
export default function BoltcardTopUp({
   ...
   */
  const getEquivalentDisplay = useCallback((value) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`p-4 rounded-lg ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {quickAdjustments.map((adj) => (
            <button
              ...
              disabled={submitting || (adj.amount > 0 && sliderValue >= maxSliderValue) || (adj.amount < 0 && sliderValue <= 0)}
              className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${
                adj.amount < 0
                  ? darkMode
        ...
        {/* Error Message */}
        {error && (
          <div className={`p-3 rounded-lg mb-4 ${darkMode ? 'bg-red-900/20' : 'bg-red-50'}`}>
        ...
        {/* Success Message */}
        {success && (
          <div className={`p-3 rounded-lg mb-4 ${darkMode ? 'bg-green-900/20' : 'bg-green-50'}`}>
              ...
              <>
                ...
                <div className={`p-3 rounded-lg border ${
                  ...
                  <p className={`text-xs ${darkMode ? 'text-blue-300' : 'text-blue-600'}`}>
                    ...
                    Card owners cannot top up their own cards via LNURL (use the slider above instead).
                  </p>
                </div>
              </>
            ) : (
