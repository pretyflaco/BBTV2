...
function parseBoltcardUrl(url) {
  try {
    ...
    if (!cardId || !p || !c) {
      console.log('[WebNFC] Missing required params - cardId:', cardId, 'p:', !!p, 'c:', !!c);
      return null;
    }
...
export default function WebNFCBalanceCheck() {
   ...
   */
  const fetchBalance = useCallback(async (cardId, p, c) => {
   ...
   */
  const handleNFCRead = useCallback((event) => {
      ...
      <div className="min-h-screen bg-black flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-400 mb-8">
            Web NFC is only supported in Chrome on Android devices. 
            Scan the QR code below to open this page on your Android phone.
          </p>
    ...
    <div className="min-h-screen bg-black flex items-center justify-center p-4">
      <div className="text-center max-w-md">
        ...
        <div className="relative w-32 h-32 mx-auto mb-8">
          ...
          <div className={`absolute inset-0 rounded-full border-2 border-yellow-500/30 ${state === State.SCANNING ? 'animate-ping' : ''}`} />
          <div className={`absolute inset-4 rounded-full border-2 border-yellow-500/50 ${state === State.SCANNING ? 'animate-ping animation-delay-150' : ''}`} />
          ...
          <div className="absolute inset-8 bg-yellow-500/20 rounded-full flex items-center justify-center">
            <svg className="w-10 h-10 text-yellow-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {/* NFC waves */}
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8.5 10.5c1-1 2.5-1 3.5 0M7 9c2-2 5-2 7 0" />
            </svg>
          </div>
        </div>
