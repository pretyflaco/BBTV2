...
interface BlinkLnAddressSetupProps {
  onComplete: (data: BlinkLnAddressData) => void
  onCancel: () => void
}
...
export default function BlinkLnAddressSetup({
  onComplete,
  onCancel,
}: BlinkLnAddressSetupProps) {
  ...
  const validateLnAddress = async (address: string) => {
    if (!address) return
    ...
    try {
      ...
      setError((err as Error).message || "Failed to validate lightning address")
      setValidating(false)
    }
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {walletInfo && (
          ...
          >
            ...
            {/* Limitation notice */}
            <div
              className={`mt-3 pt-3 border-t ${darkMode ? "border-gray-700" : "border-green-200"}`}
            >
