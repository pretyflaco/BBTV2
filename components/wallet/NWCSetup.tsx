...
interface NWCSetupProps {
  onComplete: (data: NWCCompleteData) => void
  onCancel: () => void
}

export default function NWCSetup({ onComplete, onCancel }: NWCSetupProps) {
  ...
  const validateConnection = async (uri: string) => {
    if (!uri) return
    ...
    try {
      ...
      setError((err as Error).message || "Failed to validate connection")
      setValidating(false)
    }
  }
  ...
  const looksLikeNWC =
    connectionString.trim().toLowerCase().startsWith("nostr+walletconnect://") ||
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {walletInfo && (
          <div
            className={`p-4 rounded-xl ${darkMode ? "bg-green-900/20 border-green-500/30" : "bg-green-50 border-green-200"} border`}
          >
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className={darkMode ? "text-gray-300" : "text-gray-600"}>
                <span className="font-medium">Relays:</span> {walletInfo.relays.length}{" "}
                configured
              </div>
              <div className={darkMode ? "text-gray-300" : "text-gray-600"}>
                <span className="font-medium">Capabilities:</span>
                <div className="flex flex-wrap gap-1 mt-1">
                  {walletInfo.capabilities.slice(0, 5).map((cap: string, i: number) => (
                    <span
                      key={i}
                  ...
                  {walletInfo.capabilities.length > 5 && (
                    <span
                      className={`px-2 py-0.5 rounded text-xs ${darkMode ? "bg-gray-700 text-gray-400" : "bg-gray-200 text-gray-500"}`}
                    >
      ...
      >
        ...
        <div
          className={`p-3 rounded-lg mb-3 ${darkMode ? "bg-amber-900/20 border border-amber-500/30" : "bg-amber-50 border border-amber-200"}`}
        >
          <p className={`text-xs ${darkMode ? "text-amber-200" : "text-amber-800"}`}>
            <strong>Only Blink NWC wallets are supported.</strong> This ensures zero-fee
            internal transfers. External Lightning wallets would incur routing fees that
            are difficult to account for properly.
          </p>
        </div>
