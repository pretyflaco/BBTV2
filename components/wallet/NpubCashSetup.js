...
export default function NpubCashSetup({ onComplete, onCancel, userPublicKey }) {
  ...
  const handleInputChange = (e) => {
    ...
    if (value && !value.includes('@') && (value.startsWith('npub1') || value.length > 3)) {
  ...
  return (
    <div className={`rounded-2xl p-6 ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-xl max-w-md mx-auto`}>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {addressInfo && (
          <div className={`p-4 rounded-xl ${darkMode ? 'bg-emerald-900/20 border-emerald-500/30' : 'bg-emerald-50 border-emerald-200'} border`}>
            <div className="flex items-center gap-2 mb-3">
              <svg className="w-5 h-5 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
              </svg>
              <span className={`font-medium ${darkMode ? 'text-emerald-400' : 'text-emerald-700'}`}>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className={darkMode ? 'text-gray-300' : 'text-gray-600'}>
                ...
                {addressInfo.minSats} - {addressInfo.maxSats?.toLocaleString()} sats
              </div>
            </div>

            {/* Info notice */}
            <div className={`mt-3 pt-3 border-t ${darkMode ? 'border-gray-700' : 'border-emerald-200'}`}>
              <p className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                🥜 Payments will be converted to Cashu ecash tokens. 
                Claim them anytime using your Nostr key.
              </p>
            </div>
          </div>
        )}
      ...
      <div className={`mt-6 pt-6 border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
        ...
        <div className={`mt-4 p-3 rounded-lg ${darkMode ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
          <p className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            💡 <strong>No registration needed</strong> - every valid npub has an npub.cash address automatically.
            Use a Cashu wallet like Minibits, Nutstash, or eNuts to claim your tokens.
          </p>
        </div>
      </div>
    </div>
