...
import { TEST_CREDENTIALS, API_ENDPOINTS } from './test-data';
...
export async function setupAuthenticatedState(
  ...
  const result = await page.evaluate(async ({
    ...
    const keyMaterial = await crypto.subtle.importKey(
      'raw',
      // @ts-ignore - Uint8Array works at runtime in browser context
      deviceKeyBytes,
      { name: 'PBKDF2' },
      false,
      ['deriveKey']
    ...
    // @ts-ignore - Uint8Array type mismatch in strict mode, but works at runtime
