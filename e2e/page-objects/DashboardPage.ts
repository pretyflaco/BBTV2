...
export class DashboardPage extends BasePage {
  ...
  async getRecentTransaction(): Promise<{ amount: string; type: string; date: string }> {
    const firstTransaction = this.transactionList.locator('[data-testid="transaction-item"]').first();
    
    const amount = await firstTransaction.locator('[data-testid="transaction-amount"]').textContent() || '';
    const type = await firstTransaction.locator('[data-testid="transaction-type"]').textContent() || '';
    const date = await firstTransaction.locator('[data-testid="transaction-date"]').textContent() || '';
