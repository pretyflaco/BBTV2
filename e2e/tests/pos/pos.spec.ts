...
import { TEST_DATA, TIMEOUTS } from "../../fixtures/test-data"
import { POSPage, InvoicePage } from "../../page-objects"

test.describe("Point of Sale (POS)", () => {
  let posPage: POSPage
  let _invoicePage: InvoicePage

  test.beforeEach(async ({ page }) => {
    posPage = new POSPage(page)
    _invoicePage = new InvoicePage(page)
  })
  ...
  test.describe("Numpad", () => {
    test("should display numpad on POS page", async ({ page }) => {
      ...
      const hasNumpad = await numpad
        .first()
        .isVisible()
        .catch(() => false)
      const hasNumbers = await numberButtons
        .first()
        .isVisible()
        .catch(() => false)
    ...
    test("should have all digit buttons (0-9)", async ({ page }) => {
      ...
      for (let digit = 0; digit <= 9; digit++) {
        const button = page
          .locator(`button:has-text("${digit}"), [data-testid="numpad-${digit}"]`)
          .first()
        // Button should exist
    ...
    test("should have decimal point button", async ({ page }) => {
      ...
      const _decimalButton = page.locator(
        'button:has-text("."), [data-testid="numpad-decimal"], [data-testid="numpad-."]',
      )
    ...
    test("should have clear/backspace button", async ({ page }) => {
      ...
      const _clearButton = page.locator(
        '[data-testid="clear-button"], button:has-text("C"), button:has-text("Clear"), [data-testid="numpad-clear"]',
      )
      // Should exist
    })
    ...
    test("should update amount display when pressing digits", async ({ page }) => {
      ...
      const digit1 = page
        .locator('button:has-text("1"), [data-testid="numpad-1"]')
        .first()
      const digit2 = page
        .locator('button:has-text("2"), [data-testid="numpad-2"]')
        .first()
      const amountDisplay = page
        .locator('[data-testid="amount-display"], .amount-display, [class*="amount"]')
        .first()
      ...
      if ((await digit1.isVisible()) && (await digit2.isVisible())) {
        ...
        const _displayText = await amountDisplay.textContent()
        expect(_displayText).toContain("1")
      }
    })
    ...
    test("should clear amount when clear button is pressed", async ({ page }) => {
      ...
      const digit5 = page
        .locator('button:has-text("5"), [data-testid="numpad-5"]')
        .first()
      const clearButton = page
        .locator(
          '[data-testid="clear-button"], button:has-text("C"), [data-testid="numpad-clear"]',
        )
        .first()
      const amountDisplay = page
        .locator('[data-testid="amount-display"], .amount-display, [class*="amount"]')
  ...
  test.describe("Currency Toggle", () => {
    test("should display currency toggle", async ({ page }) => {
      ...
      const _currencyToggle = page.locator(
        '[data-testid="currency-toggle"], button:has-text("BTC"), button:has-text("USD"), button:has-text("sats")',
      )
  ...
  test.describe("Invoice Generation", () => {
    ...
    test("should generate invoice when amount is entered", async ({ page }) => {
      ...
      const qrCode = page
        .locator(
          '[data-testid="invoice-qr"], canvas, svg[class*="qr"], img[alt*="QR"], svg',
        )
        .first()
    ...
    test("should display QR code for generated invoice", async ({ page }) => {
      ...
      const qrContainer = page.locator(
        '[data-testid="invoice-qr"] canvas, [data-testid="invoice-qr"] svg, [data-testid="invoice-qr"] img',
      )
      const _qrExists =
        (await qrContainer.count()) > 0 ||
        (await qrCode.locator("canvas, svg, img").count()) > 0
  ...
  test.describe("Public POS", () => {
    ...
    test("should display merchant name on public POS", async ({ page }) => {
      ...
      const pageContent = await page.content()
      const usernameVisible = pageContent
        .toLowerCase()
        .includes(TEST_DATA.usernames.staging.toLowerCase())
    ...
    test("should generate invoice in USD mode", async ({ page }) => {
      ...
      if (await currencyToggle.isVisible()) {
        ...
        if (
          initialText?.toLowerCase().includes("sat") ||
          initialText?.toLowerCase().includes("btc")
        ) {
