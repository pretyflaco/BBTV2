...
import { setupAuthenticatedState, clearAuthState, setEnvironment } from '../../fixtures/auth.fixture';
import { 
  getWalletBalance, 
...
test.describe('Wallet Setup', () => {
  ...
  test.describe('Blink Account Connection', () => {
    test('should display API key input field in settings', async ({ page }) => {
      ...
      const apiKeyInput = page.locator('[data-testid="api-key-input"], input[placeholder*="API"], input[placeholder*="key"], input[name*="api"]');
      
      // Should have some form of API key input
      const hasInput = await apiKeyInput.first().isVisible().catch(() => false);
    ...
    test('should accept valid Blink API key format', async ({ page }) => {
      ...
      const apiKeyInput = page.locator('[data-testid="api-key-input"], input[placeholder*="API"], input[name*="api"]').first();
    ...
    test('should validate API key on save', async ({ page }) => {
      ...
      const apiKeyInput = page.locator('[data-testid="api-key-input"], input[placeholder*="API"], input[name*="api"]').first();
      const saveButton = page.locator('[data-testid="save-settings"], button:has-text("Save"), button:has-text("Connect")').first();
      ...
      if (await apiKeyInput.isVisible() && await saveButton.isVisible()) {
        ...
        const successMsg = page.locator('[data-testid="success-message"], .success, [role="alert"]:has-text("success")');
        const errorMsg = page.locator('[data-testid="error-message"], .error, [role="alert"]:has-text("error")');
    ...
    test('should show connection status indicator', async ({ page }) => {
      ...
      const statusIndicator = page.locator('[data-testid="blink-connected"], [data-testid="blink-status"], .connection-status');
  ...
  test.describe('Wallet Display', () => {
    ...
    test('should update balance after transaction', async ({ page }) => {
      ...
      const initialBalance = await getWalletBalance(TEST_CREDENTIALS.apiKeys.readReceiveWrite);
      ...
      console.log('Balance updated:', initialBalance.btcBalance, '->', balanceChange.newBalance);
    });
  });
  ...
  test.describe('Transaction History', () => {
    test('should display transaction list', async ({ page }) => {
      ...
      const txList = page.locator('[data-testid="transaction-list"], .transactions, [class*="transaction"]');
    ...
    test('should paginate transactions', async ({ page }) => {
      ...
      if (hasPagination) {
        ...
        const pageContent = await page.content();
        const hasTransactions = pageContent.includes('sats') || pageContent.includes('transaction');
      ...
      try {
        ...
        try {
          ...
          await page2.evaluate((state) => {
            ...
            if (state.activeProfile) localStorage.setItem('blinkpos_active_profile', state.activeProfile);
            if (state.environment) localStorage.setItem('blink_environment', state.environment);
