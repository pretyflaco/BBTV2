 ...
 * @returns {'BLINK' | 'LN_ADDRESS' | 'LNURL'}
 */
 ...
 * @param {'BLINK' | 'LN_ADDRESS' | 'LNURL'} type - Detected type
 * @returns {string} Normalized recipient
 */
function normalizeRecipient(recipient, type) {
  const trimmed = decodeCSVString(recipient.trim());
 ...
 * @returns {{ success: boolean, records?: ParsedRecipient[], errors?: string[] }}
 */
function parseCSV(csvContent) {
  ...
  if (missingHeaders.length > 0) {
    return {
      success: false,
      errors: [`Missing required headers: ${missingHeaders.join(', ')}. Expected: recipient,amount,currency,memo`]
    };
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!validCurrencies.includes(currency)) {
      errors.push(`Row ${rowNum}: Invalid currency "${currency}". Must be SATS, USD, or BTC`);
      continue;
    }
  ...
  return {
    ...
    summary: {
      ...
      byType: {
        [RECIPIENT_TYPES.BLINK]: records.filter(r => r.type === RECIPIENT_TYPES.BLINK).length,
        [RECIPIENT_TYPES.LN_ADDRESS]: records.filter(r => r.type === RECIPIENT_TYPES.LN_ADDRESS).length,
        [RECIPIENT_TYPES.LNURL]: records.filter(r => r.type === RECIPIENT_TYPES.LNURL).length
      },
 ...
 */
function quickValidate(csvContent) {
  if (!csvContent || typeof csvContent !== 'string') {
  ...
  if (lines.length > 1001) { // 1000 recipients + header
