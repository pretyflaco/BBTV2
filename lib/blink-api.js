  ...
  */
  ...
  async sendTipViaInvoice(fromWalletId, recipientUsername, amount, memo) {
    try {
      ...
      // Step 1: Get recipient's wallet ID
      const recipientWallet = await BlinkAPI.getWalletByUsername(recipientUsername);
      if (!recipientWallet?.id) {
        throw new Error(`Could not find wallet for username: ${recipientUsername}`);
      }
      ...
      // Step 3: Pay the invoice from our wallet
      const paymentResult = await this.payLnInvoice(fromWalletId, invoice.paymentRequest);
