...
// =============================================================================
// SUNMAC VERIFICATION (per NXP AN12196 / NTAG424DNA spec)
// =============================================================================

/**
 * SV2 prefix constant for SunMAC calculation
 * Per NXP AN12196: SV2 = 3Ch||C3h||00h||01h||00h||80h||UID||SDMReadCtr
 */
 ...
 * 
 * @param {string} piccDataHex - Encrypted PICCData (32 hex chars) - used to extract UID/counter if not provided
 ...
 * @param {string} uidHex - Card UID (14 hex chars = 7 bytes) - from decrypted PICCData
 * @param {number} counter - SDM read counter (integer) - from decrypted PICCData
 * @returns {boolean} True if MAC is valid
 */
function verifySunMAC(piccDataHex, sunMacHex, k2Hex, uidHex, counter) {
  try {
    ...
    // This is equivalent to taking every other byte starting from index 1
...
/**
 * Legacy verifySunMAC for backwards compatibility (DEPRECATED)
 * This was incorrectly doing single CMAC on encrypted PICCData
 * 
 * @deprecated Use verifySunMAC with UID and counter parameters
 */
function verifySunMACLegacy(piccDataHex, sunMacHex, k2Hex) {
  console.warn('[BoltcardCrypto] verifySunMACLegacy is deprecated and incorrect');
  try {
    const piccData = Buffer.from(piccDataHex, 'hex');
    const sunMac = Buffer.from(sunMacHex, 'hex');
    const k2 = Buffer.from(k2Hex, 'hex');
...
function verifyCardTap(piccDataHex, sunMacHex, k1Hex, k2Hex, expectedCardUid = null, lastCounter = 0) {
  ...
  // (We need these for SunMAC verification)
  ...
  if (!verifySunMAC(piccDataHex, sunMacHex, k2Hex, piccResult.cardUid, piccResult.counter)) {
