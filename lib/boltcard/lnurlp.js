 ...
 * For USD cards: Converts sats to cents using current exchange rate, credits in cents
 * 
...
async function processTopUpPayment(paymentHash) {
  ...
  // For USD cards, convert sats to cents using current exchange rate
  if (currency === 'USD') {
    try {
      const BlinkAPI = require('../blink-api');
      const rate = await BlinkAPI.getExchangeRatePublic('USD');
      // rate.satPriceInUSD = price of 1 sat in USD (e.g., 0.0005 for $50k/BTC)
      ...
      console.error(`[LNURLP] Failed to get exchange rate for USD conversion: ${error.message}`);
      return { success: false, error: 'Failed to convert sats to USD' };
    }
  }
