...
async function handleTopUpRequest(cardId, serverUrl) {
  ...
  const metadata = JSON.stringify([
    ['text/plain', `Top up ${card.name || 'Boltcard'} (Note: Cannot pay from card owner's account)`],
...
async function processTopUpPayment(paymentHash) {
  ...
  if (currency === 'USD') {
    try {
      ...
      creditUnit = 'cents';
      console.log(`[LNURLP] Converting ${amountSats} sats to ${creditAmount} cents (rate: ${rate.satPriceInCurrency} cents/sat)`);
    } catch (error) {
