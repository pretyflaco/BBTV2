 ...
 * @param {string} walletId - Card's Blink wallet ID (required for invoice status check)
 * @returns {Promise<{processed: number, total: number, errors: number}>}
 */
async function checkAndProcessPendingTopUps(cardId, apiKey, environment = 'production', walletId) {
  ...
  for (const pending of pendingTopUps) {
    try {
      ...
      const status = await checkInvoiceStatus(pending.paymentHash, apiKey, apiUrl, walletId);
      ...
      if (status === 'PAID') {
        ...
        if (result.success) {
          console.log(`[LNURLP] ✅ Processed pending top-up: +${pending.amount} sats for card ${cardId}`);
          processed++;
        } else {
