 ...
 * This is stored in memory for now; could be moved to database
 * 
 * @param {string} cardId - Card ID
...
async function storePendingTopUp(cardId, paymentHash, amount, currency) {
  ...
  for (const [hash, data] of pendingTopUps.entries()) {
    if (data.createdAt < oneHourAgo) {
      pendingTopUps.delete(hash);
    }
  }
}
...
async function processTopUpPayment(paymentHash) {
  const pending = pendingTopUps.get(paymentHash);
  ...
  // Remove from pending
  pendingTopUps.delete(paymentHash);
 ...
 * @returns {object|null} Pending top-up data or null
 */
function getPendingTopUp(paymentHash) {
  return pendingTopUps.get(paymentHash) || null;
}

/**
 * Get all pending top-ups (for debugging/admin)
 * @returns {Array} Array of pending top-ups
 */
function getAllPendingTopUps() {
  return Array.from(pendingTopUps.entries()).map(([hash, data]) => ({
    paymentHash: hash,
    ...data,
  }));
}
