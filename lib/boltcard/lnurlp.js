 ...
 */
async function checkInvoiceStatus(paymentHash, apiKey, apiUrl) {
  const query = `
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`[LNURLP] Invoice status check failed: HTTP ${response.status}`);
      return 'ERROR';
    }

    const data = await response.json();
    
    if (data.errors || data.data?.lnInvoicePaymentStatus?.errors?.length > 0) {
      const errorMsg = data.errors?.[0]?.message || 
                       data.data?.lnInvoicePaymentStatus?.errors?.[0]?.message;
      console.error(`[LNURLP] Invoice status check error: ${errorMsg}`);
      return 'ERROR';
    }

    return data.data?.lnInvoicePaymentStatus?.status || 'ERROR';
  } catch (error) {
 ...
 */
async function checkAndProcessPendingTopUps(cardId, apiKey, environment = 'production') {
  ...
  for (const pending of pendingTopUps) {
    try {
      ...
      const status = await checkInvoiceStatus(pending.paymentHash, apiKey, apiUrl);
