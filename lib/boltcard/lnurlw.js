 ...
 * @returns {Promise<number>} Amount in cents (Math.round)
 */
async function convertSatsToCents(sats) {
  try {
    ...
    console.error('[LNURLW] Failed to fetch exchange rate for sats->cents:', error.message);
 ...
 * @returns {Promise<number>} Amount in sats (Math.round)
 */
async function convertCentsToSats(cents) {
  try {
    const rate = await BlinkAPI.getExchangeRatePublic('USD');
    // rate.satPriceInCurrency = cents per sat (e.g., 0.0686)
    ...
    console.error('[LNURLW] Failed to fetch exchange rate for cents->sats:', error.message);
    throw new Error('Exchange rate unavailable');
  }
}
...
async function handleWithdrawRequest(cardId, piccDataHex, sunMacHex, callbackUrl) {
  ...
  let maxWithdrawableSats;
  
  if (card.walletCurrency === 'USD') {
    try {
      maxWithdrawableSats = await convertCentsToSats(maxWithdrawable);
      console.log(`[LNURLW] USD card: converted ${maxWithdrawable} cents to ${maxWithdrawableSats} sats for LNURL`);
    } catch (error) {
      return {
        status: 'ERROR',
  ...
  console.log(`[LNURLW] Card ${cardId} tap verified. Max withdrawable: ${maxWithdrawable} ${unit} (${maxWithdrawableSats} sats)`);
 ...
 * @param {function} payInvoice - Function to pay invoice (amount, invoice, apiKey, environment, walletCurrency) -> { success, paymentHash, error }
...
async function handleWithdrawCallback(cardId, invoice, payInvoice) {
  ...
  if (card.walletCurrency === 'USD') {
    ...
    try {
      ...
      console.log(`[LNURLW] USD card: converted ${invoiceAmount.sats} sats to ${amount} cents`);
    } catch (error) {
      return {
        status: 'ERROR',
        reason: 'Failed to get exchange rate',
  ...
  const payResult = await payInvoice(invoiceAmount.sats, invoice, card.apiKey, card.environment, card.walletCurrency);
 ...
 * @returns {object|null} { sats, msats } or null
 */
