...
class BoltcardStore {
  ...
  async completePendingRegistration(registrationId, cardUid) {
    ...
    if (existingCard) {
      // If same owner, this is a re-program - increment version
      if (existingCard.ownerPubkey === registration.ownerPubkey) {
    ...
    console.log(`[BoltcardStore] Completed registration ${registrationId} -> card ${card.id}`);
