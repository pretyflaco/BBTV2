...
class BoltcardStore {
   ...
   * @param {string} cardId - Card ID
   * @returns {Promise<Array>} Array of pending top-ups for this card
   */
  async getPendingTopUpsForCard(cardId) {
    try {
      const result = await query(
        `SELECT * FROM boltcard_pending_topups 
      ...
      console.error('[BoltcardStore] getPendingTopUpsForCard error:', error.message);
      return [];
    }
  }
