...
import { formatNumber, formatBitcoinAmount, DEFAULT_BITCOIN_FORMAT } from './number-format.js';
import { CITRUSRATE_EXCLUSIVE_IDS, isCitrusrateExclusiveCurrency as isExclusive } from './citrusrate-currencies-client.js';
 ...
 */
export const STREET_RATE_CURRENCIES = [
  {
    id: 'MZN_STREET',
 ...
 * @returns {boolean}
 */
export const isStreetRateCurrency = (currencyId) => {
 ...
 * @returns {boolean}
 */
export const isCitrusrateAltCurrency = (currencyId) => {
 ...
 * @returns {boolean}
 */
export const isCitrusrateCurrency = (currencyId) => {
  return isStreetRateCurrency(currencyId) || isCitrusrateAltCurrency(currencyId) || isExclusive(currencyId);
};
 ...
 */
export const getBaseCurrencyId = (currencyId) => {
 ...
 * @returns {object|null}
 */
export const getStreetRateCurrency = (currencyId) => {
 ...
 */
export const formatCurrencyAmount = (value, currency, numberFormat = 'auto', bitcoinFormat = DEFAULT_BITCOIN_FORMAT) => {
 ...
 * @param {string} currencyId - Currency ID (e.g., 'USD', 'EUR', 'BTC', 'BTC-BIP177', 'MZN_STREET', 'KES_CITRUS')
...
export const getCurrencyById = (currencyId, currencyList) => {
  ...
  const streetCurrency = getStreetRateCurrency(currencyId);
  if (streetCurrency) {
  ...
  if (isCitrusrateAltCurrency(currencyId)) {
    const baseId = getBaseCurrencyId(currencyId); // KES_CITRUS -> KES
    const baseCurrency = currencyList.find(c => c.id === baseId);
    if (baseCurrency) {
 ...
 */
export const formatDisplayAmount = (value, currencyId, currencyList, numberFormat = 'auto', bitcoinFormat = DEFAULT_BITCOIN_FORMAT) => {
 ...
 * @param {string} bitcoinFormat - Bitcoin format used ('sats', 'bip177', 'sat')
 * @returns {object} { symbol, value, isBip177 } - Parts for custom rendering
 */
export const parseAmountParts = (formattedAmount, currencyId, bitcoinFormat = DEFAULT_BITCOIN_FORMAT) => {
