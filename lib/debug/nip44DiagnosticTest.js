...
const TEST_VECTORS = {
  ...
  hkdfSalt: hexToBytes('000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f'),
 ...
 */
export async function runNIP44Diagnostics(logCallback) {
  const log = (msg) => {
  ...
  try {
    ...
    results.tests.cryptoAvailable = { pass: false, error: e.message };
  ...
  try {
    ...
    results.tests.sha256 = { pass: false, error: e.message };
    log(`FAILED: ${e.message}`);
  }
  log('');
  ...
  try {
    ...
    results.tests.hmac = { pass: false, error: e.message };
    log(`FAILED: ${e.message}`);
  }
  log('');
  ...
  try {
    ...
    const derivedKey = hkdf(sha256, ikm, salt, info, 42);
    ...
    results.tests.hkdf = { pass: false, error: e.message };
    log(`FAILED: ${e.message}`);
  }
  log('');
  ...
  try {
    ...
    const plaintext = utf8ToBytes('Ladies and Gentlemen of the class of \'99: If I could offer you only one tip for the future, sunscreen would be it.');
    
    const cipher = chacha20(key, nonce, plaintext);
    const cipherHex = bytesToHex(cipher);
    
    // Known output from RFC 8439 section 2.4.2
    const expectedStart = '6e2e359a2568f98041ba0728dd0d6981e97e7aec1d4360c20a27afccfd9fae0bf91b65c5524733ab8f593d';
    ...
    const decrypted = chacha20(key, nonce, cipher);
    ...
    results.tests.chacha20 = { pass: false, error: e.message };
    log(`FAILED: ${e.message}`);
  }
  log('');
  ...
  try {
    ...
    const shared1 = secp256k1.getSharedSecret(sk1, '02' + pk2);
    const shared2 = secp256k1.getSharedSecret(sk2, '02' + pk1);
    
    const shared1Hex = bytesToHex(shared1.slice(1, 33)); // Remove prefix, take x-coord
    const shared2Hex = bytesToHex(shared2.slice(1, 33));
    ...
    results.tests.ecdh = { pass: false, error: e.message };
    log(`FAILED: ${e.message}`);
  }
  log('');
  ...
  try {
    ...
    const sharedPoint = secp256k1.getSharedSecret(aliceSk, '02' + bobPk);
    const sharedX = sharedPoint.slice(1, 33);
    
    // Derive conversation key using HKDF
    const conversationKey = hkdf(sha256, sharedX, utf8ToBytes('nip44-v2'), undefined, 32);
    const conversationKeyHex = bytesToHex(conversationKey);
    ...
    const plaintext = utf8ToBytes(TEST_VECTORS.plaintext);
    const ciphertext = chacha20(conversationKey, nonce, plaintext);
    ...
    const mac = hmac(sha256, conversationKey, macData);
    log(`MAC: ${bytesToHex(mac).substring(0, 32)}...`);
    
    // Now decrypt
    const decrypted = chacha20(conversationKey, nonce, ciphertext);
    const decryptedText = new TextDecoder().decode(decrypted);
    
    // Verify MAC
    const verifyMacData = concatBytes(nonce, ciphertext);
    const verifyMac = hmac(sha256, conversationKey, verifyMacData);
    const macValid = bytesToHex(mac) === bytesToHex(verifyMac);
    
    const roundTripMatch = decryptedText === TEST_VECTORS.plaintext;
    ...
    results.tests.nip44Roundtrip = { pass: false, error: e.message };
    log(`FAILED: ${e.message}`);
  ...
  try {
    ...
    const clientShared = secp256k1.getSharedSecret(clientSk, '02' + bunkerPk);
    const clientSharedX = clientShared.slice(1, 33);
    const clientConvKey = hkdf(sha256, clientSharedX, utf8ToBytes('nip44-v2'), undefined, 32);
    
    // Bunker computes shared secret with client's pubkey
    const bunkerShared = secp256k1.getSharedSecret(bunkerSk, '02' + clientPk);
    const bunkerSharedX = bunkerShared.slice(1, 33);
    const bunkerConvKey = hkdf(sha256, bunkerSharedX, utf8ToBytes('nip44-v2'), undefined, 32);
    
    // Keys should match
    const keysMatch = bytesToHex(clientConvKey) === bytesToHex(bunkerConvKey);
    ...
    const message = '{"method":"connect","params":["test"]}';
    const nonce = new Uint8Array(12);
    crypto.getRandomValues(nonce);
    
    const encrypted = chacha20(clientConvKey, nonce, utf8ToBytes(message));
    const mac = hmac(sha256, clientConvKey, concatBytes(nonce, encrypted));
    
    // Bunker decrypts message
    const decrypted = chacha20(bunkerConvKey, nonce, encrypted);
    const decryptedMsg = new TextDecoder().decode(decrypted);
    
    // Bunker verifies MAC
    const verifyMac = hmac(sha256, bunkerConvKey, concatBytes(nonce, encrypted));
    const macValid = bytesToHex(mac) === bytesToHex(verifyMac);
    
    const messageMatch = decryptedMsg === message;
    ...
    results.tests.crossKeypair = { pass: false, error: e.message };
    log(`FAILED: ${e.message}`);
  }
  log('');
 ...
 * @param {Object} results - Test results from runNIP44Diagnostics
 */
export async function sendDiagnosticsToServer(results) {
  try {
    const response = await fetch('/api/debug/remote-log', {
      method: 'POST',
    ...
    console.warn('[NIP44-Diag] Failed to send results to server:', e.message);
  }
}
