...
interface LogoLoadOptions {
  paperWidth?: number
  threshold?: number
  invert?: boolean
}

interface LogoBitmapResult {
  bitmap: Uint8Array
  width: number
  height: number
  bytesPerRow: number
...
const logoCache: Map<string, LogoBitmapResult> = new Map()

/**
 * Configuration for different paper widths
 */
const PAPER_CONFIGS: Record<number, PaperConfig> = {
  80: {
...
export async function loadLogoForPrint(
  logoUrl: string,
  options: LogoLoadOptions = {},
): Promise<LogoBitmapResult> {
...
export async function preloadLogo(
  logoUrl: string,
  paperWidths: number[] = [58, 80],
): Promise<void> {
  const promises = paperWidths.map((width) =>
    loadLogoForPrint(logoUrl, { paperWidth: width }).catch((err: unknown) => {
      console.warn(
        `[LogoRasterizer] Failed to preload logo for ${width}mm:`,
 ...
 */
export function getBlinkLogoUrl(): string {
 ...
 */
export function bitmapToESCPOS(
  bitmap: Uint8Array,
  width: number,
  height: number,
  mode: number = 0,
...
export type { PaperConfig, LogoLoadOptions, LogoBitmapResult }
