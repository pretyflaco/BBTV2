 ...
 */
const ERROR_CORRECTION: Record<string, string> = {
  L: "L", // 7% recovery
  M: "M", // 15% recovery
  Q: "Q", // 25% recovery
  H: "H", // 30% recovery
 ...
 */
const DEFAULT_OPTIONS: Required<QRRasterOptions> = {
  errorCorrection: "M",
...
class QRCodeRasterizer {
  ...
  ): Promise<QRBitmapResult> {
    const opts: Required<QRRasterOptions> = { ...this.options, ...options }
   ...
   */
  async generateMatrix(data: string, options: QRRasterOptions = {}): Promise<number[][]> {
    const opts: Required<QRRasterOptions> = { ...this.options, ...options }
 ...
 * @returns {{bitmap: Uint8Array, width: number, height: number}}
 */
function matrixToBitmap(matrix: (boolean | number)[][], scale: number = 1): BitmapResult {
...
export type { QRRasterOptions, QRBitmapResult, BitmapResult, QRSizeEstimate }
