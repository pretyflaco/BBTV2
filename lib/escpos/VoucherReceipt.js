...
class VoucherReceipt {
  ...
  build(voucher, options = {}) {
    ...
    if (isUsdVoucher && voucher.usdAmountCents) {
      ...
      if (voucher.displayAmount && voucher.displayCurrency && voucher.displayCurrency !== 'USD') {
        b.labelValue('Price:', formatPrice(voucher.displayAmount, voucher.displayCurrency), { labelWidth });
      }
    } else {
  ...
  _buildInfoSection(voucher, opts, compact) {
    ...
    const isUsdVoucher = voucher.walletCurrency === 'USD';
    const isBtcDisplayCurrency = !voucher.displayCurrency || 
    ...
    if (isUsdVoucher) {
      ...
      if (isBtcDisplayCurrency) {
        ...
        if (voucher.usdAmountCents && voucher.commissionPercent > 0) {
          // Calculate original USD price before commission was deducted
          const originalUsdCents = Math.round(voucher.usdAmountCents / (1 - voucher.commissionPercent / 100));
