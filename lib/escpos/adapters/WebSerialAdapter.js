...
class WebSerialAdapter extends BaseAdapter {
  ...
  async connect(connectionOptions = {}) {
    ...
    try {
      ...
      this.printerInfo = {
        ...
        vendorName: PRINTER_VENDORS.find(v => v.vendorId === info.usbVendorId)?.name || 'Unknown',
      };
  ...
  async discover() {
    ...
    return ports.map((port, index) => {
      const info = port.getInfo();
      const vendor = PRINTER_VENDORS.find(v => v.vendorId === info.usbVendorId);
   ...
   * @returns {Array}
   */
  static getPrinterVendors() {
    return [...PRINTER_VENDORS];
  }
}

export default WebSerialAdapter;
export { WebSerialAdapter, SERIAL_CONFIGS, PRINTER_VENDORS };
