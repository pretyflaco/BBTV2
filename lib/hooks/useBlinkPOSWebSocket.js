...
export function useBlinkPOSWebSocket(userApiKey, userWalletId, onPaymentReceived) {
  ...
  const scheduleReconnect = () => {
    ...
    if (reconnectAttempts >= 10) {
      console.log('❌ BlinkPOS WebSocket connection abandoned after 10 consecutive failures - possible service outage.');
      const delay = 60000; // 1 minute delay after 10 failures
      console.log(`🔄 BlinkPOS - Scheduling reconnect in ${delay}ms (attempt ${reconnectAttempts + 1}) - service outage mode`);
