 ...
 * @param {string} expectedPaymentHash - CRITICAL: The payment hash of the client's pending invoice
 *                                       Only trigger onPaymentReceived callback if payment matches
 */
export function useBlinkPOSWebSocket(userApiKey, userWalletId, onPaymentReceived, nwcOptions = {}, blinkLnAddressOptions = {}, npubCashOptions = {}, expectedPaymentHash = null) {
  ...
  // CRITICAL: Track expected payment hash to only trigger callback for THIS client's invoice
  // Use ref to allow immediate updates without waiting for React re-render
  const expectedPaymentHashRef = useRef(expectedPaymentHash);
  
  // Function to immediately set expected payment hash (bypasses React render cycle)
  const setExpectedPaymentHash = (hash) => {
    expectedPaymentHashRef.current = hash;
    if (hash) {
      console.log(`🎯 [BlinkPOS WS] Expected payment hash SET: ${hash.substring(0, 16)}...`);
    } else {
      console.log('🎯 [BlinkPOS WS] Expected payment hash CLEARED');
    }
  };
  ...
  // Update expected payment hash ref when it changes
  useEffect(() => {
    expectedPaymentHashRef.current = expectedPaymentHash;
    if (expectedPaymentHash) {
      console.log(`🎯 [BlinkPOS WS] Tracking payment hash: ${expectedPaymentHash.substring(0, 16)}...`);
    }
  }, [expectedPaymentHash]);
  ...
  // CRITICAL: Helper to check if payment matches expected hash before triggering callback
  // This prevents showing success animation on wrong client when multiple devices use same BlinkPOS account
  // Defined at hook level so it can be used in both WebSocket handler and forwardPayment
  const isExpectedPayment = (paymentHash) => {
    const expected = expectedPaymentHashRef.current;
    
    console.log('🔍 Checking payment match:', {
      receivedHash: paymentHash ? paymentHash.substring(0, 16) + '...' : 'NONE',
      expectedHash: expected ? expected.substring(0, 16) + '...' : 'NONE'
    });
    
    if (!expected) {
      console.log('⚠️ No expected payment hash set - this client has no pending invoice');
      return false;
    }
    
    if (!paymentHash) {
      console.log('⚠️ No payment hash in received payment - cannot verify');
      return false;
    }
    
    const matches = paymentHash === expected;
    if (matches) {
      console.log('✅ Payment hash MATCHES - this is our payment!');
    } else {
      console.log(`ℹ️ Payment ${paymentHash.substring(0, 16)}... does not match expected ${expected.substring(0, 16)}... - skipping callback`);
    }
    return matches;
  };
  ...
  const forwardPayment = async (paymentData) => {
    ...
    // Note: isExpectedPayment helper is now defined at hook level (above forwardPayment)
    // Animation is triggered immediately in WebSocket handler, so we don't trigger it here
    ...
    try {
        ...
        if (paymentData.paymentHash) {
          ...
          if (tipResponse.ok) {
            ...
            if (tipResult.skipForwarding || tipResult.alreadyProcessed || tipResult.alreadyProcessing || tipResult.noPaymentData) {
              ...
              // Don't forward - another process is handling it
              return;
            }
            ...
            // API error - DO NOT forward to prevent potential double-spend
            // The payment might have been processed by another handler
            console.log('⚠️ NWC tip API failed - skipping forwarding to prevent duplicate');
            return;
          }
        }
      ...
      // BLINK LIGHTNING ADDRESS FORWARDING PATH
        ...
        if (response.status === 409) {
          ...
          // Note: Animation already triggered instantly in WebSocket handler, no need to trigger again
          return;
        }
      }
      ...
      // BLINK API KEY FORWARDING PATH
        ...
        if (paymentData.paymentHash) {
          ...
          // No payment hash available - this is unusual and risky
          // Without a payment hash, we cannot use atomic claim mechanism
          // This could lead to duplicate payments if multiple processes try to forward
          console.warn('⚠️ CRITICAL: No payment hash available - cannot use atomic claim!');
          console.warn('⚠️ Skipping forwarding to prevent potential duplicate payment');
          console.warn('⚠️ Payment will be handled by webhook if configured');
          // DO NOT forward without claim mechanism - webhook will handle it if payment hash is available there
          // If webhook also doesn't have hash, the payment simply won't be forwarded (safer than double-spend)
          return;
        }
      }
      ...
      if (result.success) {
        ...
        // Note: Animation already triggered instantly in WebSocket handler when payment was detected
        // No need to trigger callback again here - forwarding is just background processing
      } else {
          ...
          try {
            ...
            if (message.type === 'connection_ack') {
              ...
              if (transaction && transaction.direction === 'RECEIVE' && transaction.status === 'SUCCESS') {
                // Extract payment hash from multiple possible locations
                // 1. LnUpdate level paymentHash (preferred)
                // 2. Transaction initiationVia paymentHash (fallback for LN payments)
                const lnUpdatePaymentHash = message.payload?.data?.myUpdates?.update?.paymentHash;
                const initiationPaymentHash = transaction.initiationVia?.paymentHash;
                const paymentHash = lnUpdatePaymentHash || initiationPaymentHash;
                
                console.log('🎉 BLINKPOS PAYMENT DETECTED!', {
                  id: transaction.id,
                  amount: transaction.settlementAmount,
                  memo: transaction.memo,
                  paymentHash: paymentHash ? paymentHash.substring(0, 16) + '...' : 'NONE',
                  expectedPaymentHash: expectedPaymentHashRef.current ? expectedPaymentHashRef.current.substring(0, 16) + '...' : 'NONE'
                });
                
                // INSTANT UX: Trigger success animation IMMEDIATELY when customer payment is detected
                // This provides instant feedback to the customer before forwarding completes
                if (onPaymentReceived && isExpectedPayment(paymentHash)) {
                  console.log('🎬 Triggering INSTANT payment animation (before forwarding)');
                  onPaymentReceived({
                    amount: transaction.settlementAmount,
                    currency: transaction.settlementCurrency || 'BTC',
                    memo: transaction.memo,
                    isForwarded: false,  // Mark as not-yet-forwarded (animation triggered early)
                    paymentHash: paymentHash
                  });
                }
                
                // Forward payment in background (customer already saw success animation)
                forwardPayment({
                  id: transaction.id,
                  amount: transaction.settlementAmount,
  ...
  return {
    ...
    manualReconnect,
    reconnectAttempts,
    setExpectedPaymentHash  // Immediately set expected hash without waiting for React re-render
  };
}
