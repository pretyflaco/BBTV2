 ...
 */
export function useBlinkPOSWebSocket(userApiKey, userWalletId, onPaymentReceived, nwcOptions = {}, blinkLnAddressOptions = {}) {
  ...
  }, [userApiKey, userWalletId, nwcOptions?.isActive, blinkLnAddressOptions?.isActive, blinkLnAddressOptions?.username]);
  ...
  const forwardPayment = async (paymentData) => {
    ...
    // CRITICAL: Validate we have either Blink API key, NWC, or Lightning Address credentials
    if (!useNwc && !useBlinkLnAddress && (!currentApiKey || !currentWalletId)) {
    ...
    console.log('💰 PAYMENT FORWARDING ATTEMPT:', {
      ...
      apiKeyPrefix: useNwc || useBlinkLnAddress ? 'N/A' : currentApiKey?.substring(0, 10) + '...',
      timestamp: new Date().toISOString()
    });
    ...
    try {
      console.log('🔄 Forwarding payment immediately:', {
        paymentId: paymentData.id,
        amount: paymentData.amount,
        destination: useNwc ? 'NWC Wallet' : currentWalletId
      });
      ...
      if (useNwc) {
        ...
        const nwcInvoiceResult = await currentNwc.makeInvoice({
  ...
  useEffect(() => {
    ...
    // Check if we have a valid forwarding destination (Blink API key, NWC, or Lightning Address)
    ...
    if (!hasBlinkCredentials && !hasNwcDestination && !hasBlinkLnAddress) {
