 ...
 */
export function useBlinkPOSWebSocket(userApiKey, userWalletId, onPaymentReceived, nwcOptions = {}) {
  ...
  }, [userApiKey, userWalletId, nwcOptions?.isActive]);
  ...
  const forwardPayment = async (paymentData) => {
    ...
    // CRITICAL: Validate we have either Blink or NWC credentials
    if (!useNwc && (!currentApiKey || !currentWalletId)) {
    ...
    console.log('💰 PAYMENT FORWARDING ATTEMPT:', {
      ...
      destination: useNwc ? 'NWC Wallet' : 'Blink Wallet',
      toWalletId: useNwc ? 'NWC' : currentWalletId,
      apiKeyPrefix: useNwc ? 'N/A (NWC)' : currentApiKey?.substring(0, 10) + '...',
      timestamp: new Date().toISOString()
    });
  ...
  useEffect(() => {
    ...
    console.log('✅ BlinkPOS WebSocket: Valid forwarding destination found', {
      destination: hasNwcDestination ? 'NWC' : 'Blink',
      hasBlinkCredentials,
      hasNwcDestination
    });
