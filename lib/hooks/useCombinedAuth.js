...
export function useCombinedAuth() {
  ...
  const nwc = useNWC(nostrAuth.publicKey);
  ...
  const needsWalletSetup = useMemo(() => {
  ...
  // Works with both NIP-98 session AND pubkey-based auth (for external signers)
  const storeBlinkAccountOnServer = useCallback(async (apiKey, preferredCurrency = 'BTC', label = null) => {
    console.log('[storeBlinkAccountOnServer] Called with authMode:', authMode, 'hasServerSession:', nostrAuth.hasServerSession, 'publicKey:', nostrAuth.publicKey);
    ...
    // For external signers (no server session), use pubkey-based auth
    const usePubkeyAuth = !nostrAuth.hasServerSession && nostrAuth.publicKey;
    
    if (!nostrAuth.hasServerSession && !nostrAuth.publicKey) {
      console.log('[storeBlinkAccountOnServer] Skipping - no session and no pubkey');
      return { success: false, error: 'No authentication available' };
    }

    try {
      console.log('[storeBlinkAccountOnServer] Making POST request... (pubkey auth:', usePubkeyAuth, ')');
      
      const body = { apiKey, preferredCurrency, label };
      if (usePubkeyAuth) {
        body.pubkey = nostrAuth.publicKey;
      }
  ...
  }, [authMode, nostrAuth.hasServerSession, nostrAuth.publicKey]);
