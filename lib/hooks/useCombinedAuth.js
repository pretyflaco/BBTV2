...
export function useCombinedAuth() {
  ...
  const storeBlinkAccountOnServer = useCallback(async (apiKey, preferredCurrency = 'BTC', label = null) => {
    ...
    if (!nostrAuth.hasServerSession) {
      console.log('[storeBlinkAccountOnServer] Skipping server storage - no NIP-98 session (external signer)');
      ...
      return { success: false, error: 'Server storage requires NIP-98 session', localOnly: true };
    }

    try {
      console.log('[storeBlinkAccountOnServer] Making POST request with NIP-98 session...');
