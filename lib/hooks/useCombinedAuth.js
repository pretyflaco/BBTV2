...
export function useCombinedAuth() {
  ...
  const authMode = useMemo(() => {
    if (legacyAuth.user) return 'legacy';
  ...
  const user = useMemo(() => {
    ...
    if (nostrAuth.isAuthenticated) {
      // Authenticated with Nostr but no Blink account yet
      return {
        username: null,
        preferredCurrency: profile.preferences?.defaultCurrency || 'BTC',
        publicKey: nostrAuth.publicKey,
  ...
  const storeBlinkAccountOnServer = useCallback(async (apiKey, preferredCurrency = 'BTC') => {
    ...
    try {
      const response = await fetch('/api/auth/nostr-blink-account', {
        ...
        credentials: 'include',
        body: JSON.stringify({ apiKey, preferredCurrency })
      });
      ...
      console.error('Failed to store Blink account on server:', error);
