...
export function NostrAuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    return () => {
      console.log('[useNostrAuth] PROVIDER UNMOUNTED, instance:', providerInstanceId.current);
    };
  }, []);
  ...
  const updateState = useCallback((updates) => {
    setState(prev => {
      ...
      if (updates.hasOwnProperty('isAuthenticated') || newState.isAuthenticated !== prev.isAuthenticated) {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      console.log('[useNostrAuth] BUILD VERSION: v24-manual-step2');
      console.log('[useNostrAuth] checkAuth() ENTRY - instance:', providerInstanceId.current, 'authCheckInitiated:', authCheckInitiated.current, 'challengeFlowHandling:', challengeFlowHandling.current);
      ...
      try {
        ...
        const hasPending = NostrAuthService.hasPendingChallengeFlow();
        console.log('[useNostrAuth] hasPendingChallengeFlow result:', hasPending);
        
        if (hasPending) {
          console.log('[useNostrAuth] INSIDE pending challenge block, challengeFlowHandling.current =', challengeFlowHandling.current);
          ...
          console.log('[useNostrAuth] challengeFlowHandling.current is now:', challengeFlowHandling.current);
          ...
          if (method === 'nostrConnect') {
            ...
            setTimeout(async () => {
              try {
                ...
                const NostrConnectService = (await import('../nostr/NostrConnectService.js')).default;
                
                const restoreResult = await NostrConnectService.restoreSession();
                
                if (!restoreResult.success) {
                  console.warn('[useNostrAuth] Nostr Connect: Failed to restore session:', restoreResult.error);
                ...
                console.log('[useNostrAuth] Nostr Connect: NIP-98 login result:', sessionResult);
                
                if (sessionResult.success) {
                  console.log('[useNostrAuth] ✓ Nostr Connect: Server session established');
                  updateState({ hasServerSession: true });
                  ...
                  console.warn('[useNostrAuth] Nostr Connect: NIP-98 login failed:', sessionResult.error);
                }
              } catch (e) {
    ...
    if (typeof window !== 'undefined') {
      console.log('[useNostrAuth] useEffect running, authCheckInitiated.current =', authCheckInitiated.current);
      ...
      console.log('[useNostrAuth] Setting authCheckInitiated.current = true, calling checkAuth()');
  ...
  const checkPendingSignerFlow = useCallback(async () => {
    console.log('[useNostrAuth] checkPendingSignerFlow() called, challengeFlowHandling:', challengeFlowHandling.current);
    ...
    if (NostrAuthService.hasPendingChallengeFlow()) {
      ...
      if (challengeFlowHandling.current) {
        console.log('[useNostrAuth] checkPendingSignerFlow: Challenge flow already being handled, skipping');
        return { pending: true }; // Return pending to indicate flow is in progress
      }
      challengeFlowHandling.current = true;
      console.log('[useNostrAuth] checkPendingSignerFlow: Set challengeFlowHandling = true');
      ...
      console.log('[useNostrAuth] checkPendingSignerFlow - nostrReturn:', nostrReturn?.substring(0, 30) || 'null');
      console.log('[useNostrAuth] checkPendingSignerFlow - flow.step:', flow?.step || 'none');
      console.log('[useNostrAuth] checkPendingSignerFlow - flow.pubkey:', flow?.pubkey?.substring(0, 16) || 'none');
      ...
      // Note: Amber concatenates results directly, so we get "challenge{pubkey}" or "signed{event}"
      if (nostrReturn?.startsWith('challenge')) {
        console.log('[useNostrAuth] checkPendingSignerFlow - BRANCH: challenge return (Step 1 complete)');
        result = await NostrAuthService.handleChallengeFlowReturn();
        console.log('[useNostrAuth] checkPendingSignerFlow - handleChallengeFlowReturn result:', JSON.stringify(result));
      } else if (nostrReturn?.startsWith('signed')) {
        console.log('[useNostrAuth] checkPendingSignerFlow - BRANCH: signed return (Step 2 complete)');
        result = await NostrAuthService.handleChallengeSignReturn();
        console.log('[useNostrAuth] checkPendingSignerFlow - handleChallengeSignReturn result:', JSON.stringify(result));
      } else {
        // No nostr_return in URL - check what state we're in
        console.log('[useNostrAuth] checkPendingSignerFlow - BRANCH: no nostr_return in URL');
        if (flow?.step === 'awaitingSignedChallenge') {
          // v24: We're waiting for the signed challenge but URL doesn't have it
          // This means user returned from Amber after Step 1 (pubkey granted)
          // Instead of auto-redirecting (which fails on Bitcoinize device),
          // return a flag so UI can show a manual "Continue to Amber" button
          console.log('[useNostrAuth] checkPendingSignerFlow - Flow is awaitingSignedChallenge but no signed event in URL');
          console.log('[useNostrAuth] checkPendingSignerFlow - v24: Returning needsManualStep2 for user-initiated navigation');
          ...
          console.log('[useNostrAuth] checkPendingSignerFlow - Stale flow in awaitingPubkey state, clearing');
          NostrAuthService.clearPendingChallengeFlow();
          result = { success: false, error: 'Stale challenge flow, please try again' };
        } else {
          console.log('[useNostrAuth] checkPendingSignerFlow - Unknown flow state:', flow?.step);
          result = { success: false, error: 'Unknown challenge flow state' };
        }
      }
      ...
      if (result.pending) {
        ...
        console.log('[useNostrAuth] checkPendingSignerFlow - Result is pending, keeping challengeFlowHandling = true');
        updateState({ loading: false });
        return { pending: true };
      }
      ...
      // Flow completed (success or failure) - reset the flag
      challengeFlowHandling.current = false;
      console.log('[useNostrAuth] checkPendingSignerFlow: Reset challengeFlowHandling = false (flow completed)');
   ...
   * 
   * v29: BLOCKING flow - waits for NIP-98 session establishment before setting isAuthenticated.
   ...
   * @param {Function} options.onProgress - Progress callback: (stage, message) => void
   *   Stages: 'signing' | 'syncing' | 'complete'
   * @param {number} options.timeout - Timeout in ms (default 30000)
   * @returns {Promise<{success: boolean, profile?: Object, error?: string, errorType?: string}>}
   */
  const signInWithNostrConnect = useCallback(async (publicKey, options = {}) => {
    const { onProgress, timeout = 30000 } = options;
    
    console.log('[useNostrAuth] signInWithNostrConnect v29 called, pubkey:', publicKey?.slice(0, 16) + '...');
    // Note: Don't set loading state here - modal handles its own UI

    try {
      // Step 1: Register with NostrAuthService (local state only)
      const result = NostrAuthService.signInWithNostrConnect(publicKey);
      ...
      const profile = ProfileStorage.createProfile(publicKey, 'nostrConnect');
      ProfileStorage.setActiveProfile(profile.id);
      const activeBlinkAccount = profile.blinkAccounts.find(a => a.isActive) || null;

      // Step 3: NIP-98 login (this is the slow part - relay signing)
      console.log('[useNostrAuth] v29: Starting NIP-98 login (blocking)...');
      ...
      if (!sessionResult.success) {
        console.warn('[useNostrAuth] v29: NIP-98 login failed:', sessionResult.error);
        return { 
          success: false, 
      ...
      console.log('[useNostrAuth] v29: ✓ Server session established');
      
      // Step 4: Sync data from server
      console.log('[useNostrAuth] v29: Syncing data from server...');
      onProgress?.('syncing', 'Loading your data...');
      
      try {
        const syncResult = await syncBlinkAccountFromServer();
        console.log('[useNostrAuth] v29: Sync result:', syncResult);
      } catch (syncError) {
        // Soft failure - don't block sign-in for sync issues
        console.warn('[useNostrAuth] v29: Sync failed (non-blocking):', syncError);
      }
      
      // Step 5: NOW set authenticated state (after session is established)
      console.log('[useNostrAuth] v29: Setting authenticated state...');
      updateState({
        loading: false,
        isAuthenticated: true,
        publicKey: publicKey.toLowerCase(),
      ...
      console.log('[useNostrAuth] v29: ✓ Nostr Connect sign-in complete');
      return { success: true, profile, publicKey };
      
    } catch (error) {
      console.error('[useNostrAuth] v29: Nostr Connect sign-in failed:', error);
