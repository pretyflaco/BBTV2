    ...
    const checkAuth = async () => {
      try {
        ...
        if (NostrAuthService.hasPendingChallengeFlow()) {
          ...
          // Use captured URL params to avoid race condition where URL gets cleaned
          // before all concurrent checkAuth calls can read it
          const nostrReturn = capturedUrlParams.current?.nostrReturn;
          console.log('[useNostrAuth] Using captured nostr_return:', nostrReturn?.substring(0, 20) || 'none');
