...
/**
 * IndexedDB helper to get signer return data stored by Service Worker
 * This is the key fix for Android - SW captures params before page loads
 */
const DB_NAME = 'blink-pos-signer';
const DB_VERSION = 1;
const STORE_NAME = 'signer-returns';

async function getSignerReturnFromIndexedDB() {
  return new Promise((resolve) => {
    try {
      const request = indexedDB.open(DB_NAME, DB_VERSION);
      request.onerror = () => {
        console.log('[useNostrAuth] IndexedDB open error');
        resolve(null);
      };
      ...
      request.onsuccess = () => {
        ...
        try {
          ...
          getRequest.onsuccess = () => {
            ...
            // Only return if fresh (within 2 minutes)
            if (data && Date.now() - data.timestamp < 2 * 60 * 1000) {
              console.log('[useNostrAuth] Got signer return from IndexedDB:', data.nostrReturn?.substring(0, 30));
              resolve(data);
            } else {
              resolve(null);
            }
      ...
      console.log('[useNostrAuth] IndexedDB error:', e);
      resolve(null);
    }
  });
}

async function clearSignerReturnFromIndexedDB() {
  return new Promise((resolve) => {
    try {
      const request = indexedDB.open(DB_NAME, DB_VERSION);
      ...
      request.onsuccess = () => {
        ...
        try {
          ...
          store.delete('latest');
          tx.oncomplete = () => {
            console.log('[useNostrAuth] Cleared signer return from IndexedDB');
            db.close();
            resolve();
          };
          tx.onerror = () => {
...
export function NostrAuthProvider({ children }) {
  const [state, setState] = useState({
    ...
    error: null,
    pendingAmberApproval: false // Waiting for user to approve in Amber app
  });
  ...
  // Capture URL params immediately on mount (before any async operations can clean them)
  const capturedUrlParams = useRef(null);
  // Also track IndexedDB data (set asynchronously)
  const indexedDBData = useRef(null);
  
  if (typeof window !== 'undefined' && capturedUrlParams.current === null) {
    const urlParams = new URLSearchParams(window.location.search);
    capturedUrlParams.current = {
      nostrReturn: urlParams.get('nostr_return'),
      captured: true
    };
    console.log('[useNostrAuth] Captured URL params on mount:', capturedUrlParams.current.nostrReturn?.substring(0, 20) || 'none');
  }
  
  // Listen for Service Worker messages (real-time signer return notifications)
  useEffect(() => {
    if (typeof window === 'undefined' || !navigator.serviceWorker) return;
    
    const handleSWMessage = async (event) => {
      if (event.data?.type === 'SIGNER_RETURN') {
        console.log('[useNostrAuth] *** RECEIVED SIGNER_RETURN FROM SERVICE WORKER ***');
        console.log('[useNostrAuth] SW data:', event.data.nostrReturn?.substring(0, 30));
        
        // Store in ref for immediate use
        indexedDBData.current = {
          nostrReturn: event.data.nostrReturn,
          fromSW: true
        };
        
        // If we have a pending flow, try to complete it
        if (NostrAuthService.hasPendingChallengeFlow()) {
          console.log('[useNostrAuth] Have pending flow, attempting to complete with SW data...');
          
          const nostrReturn = event.data.nostrReturn;
          let result;
          
          if (nostrReturn?.startsWith('challenge')) {
            result = await NostrAuthService.handleChallengeFlowReturn(nostrReturn);
          } else if (nostrReturn?.startsWith('signed')) {
            result = await NostrAuthService.handleChallengeSignReturn(nostrReturn);
          }
          
          if (result?.success && result?.publicKey) {
            console.log('[useNostrAuth] ✓ Successfully completed auth from SW message!');
            // Clear IndexedDB data since we used it
            await clearSignerReturnFromIndexedDB();
    ...
    const checkAuth = async () => {
      try {
        ...
        if (NostrAuthService.hasPendingChallengeFlow()) {
          ...
          // PRIORITY ORDER for getting nostr_return:
          // 1. IndexedDB (Service Worker captured it before page load - most reliable on Android)
          // 2. Captured URL params (synchronous capture on mount)
          // 3. Current URL params (fallback)
          
          let nostrReturn = capturedUrlParams.current?.nostrReturn;
          console.log('[useNostrAuth] URL captured nostr_return:', nostrReturn?.substring(0, 20) || 'none');
          
          // Check IndexedDB for Service Worker captured data (key Android fix)
          if (!nostrReturn) {
            const idbData = await getSignerReturnFromIndexedDB();
            if (idbData?.nostrReturn) {
              nostrReturn = idbData.nostrReturn;
              console.log('[useNostrAuth] *** GOT nostr_return FROM INDEXEDDB (SW capture) ***:', nostrReturn.substring(0, 30));
              // Clear it so we don't reuse stale data
              await clearSignerReturnFromIndexedDB();
            }
          }
          
          console.log('[useNostrAuth] Final nostr_return:', nostrReturn?.substring(0, 20) || 'none');
          
          let result;
          // Note: Amber concatenates results directly, so we get "challenge{pubkey}" or "signed{event}"
          // Pass captured nostrReturn to avoid race condition where URL gets cleaned
          if (nostrReturn?.startsWith('challenge')) {
            result = await NostrAuthService.handleChallengeFlowReturn(nostrReturn);
          } else if (nostrReturn?.startsWith('signed')) {
            result = await NostrAuthService.handleChallengeSignReturn(nostrReturn);
          } else {
            // No nostr_return in URL - check what step we're in
            const flow = NostrAuthService.getPendingChallengeFlow();
            if (flow?.step === 'awaitingSignedChallenge') {
              // We have a pending flow waiting for signed challenge
              // Try to complete it (will attempt clipboard fallback)
              console.log('[useNostrAuth] Awaiting signed challenge - trying to complete without URL params');
              result = await NostrAuthService.handleChallengeSignReturn(null);
              
              // If still failed (clipboard didn't have the event), show waiting state
              if (!result.success && !result.publicKey) {
                console.log('[useNostrAuth] Could not complete sign-in, showing waiting state');
                updateState({ 
                  loading: false,
                  error: result.error || null,
                  pendingAmberApproval: true
                });
                return;
              }
            } else if (flow?.step === 'awaitingPubkey') {
              // Still waiting for pubkey from Amber - also show waiting state
              console.log('[useNostrAuth] Awaiting pubkey from Amber - user needs to approve in Amber app');
              updateState({ 
                loading: false, 
                error: null,
                pendingAmberApproval: true
  ...
  const checkPendingSignerFlow = useCallback(async () => {
    ...
    if (NostrAuthService.hasPendingChallengeFlow()) {
      ...
      // Use captured params if available (to avoid race conditions)
      // Fall back to reading from URL if not captured
      const nostrReturn = capturedUrlParams.current?.nostrReturn || 
        new URLSearchParams(window.location.search).get('nostr_return');
      console.log('[useNostrAuth] checkPendingSignerFlow using nostr_return:', nostrReturn?.substring(0, 20) || 'none');
      
      updateState({ loading: true });
      
      let result;
      // Note: Amber concatenates results directly, so we get "challenge{pubkey}" or "signed{event}"
      // Pass nostrReturn to avoid race condition where URL gets cleaned
      if (nostrReturn?.startsWith('challenge')) {
        result = await NostrAuthService.handleChallengeFlowReturn(nostrReturn);
      } else if (nostrReturn?.startsWith('signed')) {
        result = await NostrAuthService.handleChallengeSignReturn(nostrReturn);
