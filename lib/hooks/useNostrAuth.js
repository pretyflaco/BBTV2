      ...
      try {
        ...
        if (publicKey && method) {
          ...
          if (method === 'externalSigner') {
            ...
            // For external signers, try to sync by pubkey instead
            setTimeout(async () => {
              try {
                const syncResponse = await fetch(`/api/auth/nostr-blink-account?pubkey=${publicKey}`, {
                  method: 'GET'
                });
                if (syncResponse.ok) {
                  const data = await syncResponse.json();
                  if (data.hasAccount && data.blinkUsername && data.apiKey) {
                    console.log('[useNostrAuth] Found Blink account on server for external signer:', data.blinkUsername);
                    const currentProfile = ProfileStorage.loadProfile(publicKey);
                    if (currentProfile) {
                      const existingAccount = currentProfile.blinkAccounts.find(a => a.username === data.blinkUsername);
                      if (!existingAccount) {
                        const encryptedApiKey = await CryptoUtils.encryptWithDeviceKey(data.apiKey);
                        const serverAccount = {
                          id: `server-${Date.now()}`,
                        ...
                        const updatedAccounts = [...currentProfile.blinkAccounts, serverAccount];
                        const updatedProfile = { ...currentProfile, blinkAccounts: updatedAccounts };
                        ProfileStorage.updateProfile(updatedProfile);
                        const newActiveBlinkAccount = serverAccount.isActive ? serverAccount : 
                          (updatedAccounts.find(a => a.isActive) || null);
                        updateState({ profile: updatedProfile, activeBlinkAccount: newActiveBlinkAccount });
                        console.log('[useNostrAuth] ✓ Synced Blink account from server for external signer');
                      }
                    }
                  }
                }
              } catch (e) {
                console.warn('[useNostrAuth] External signer pubkey sync failed:', e);
              }
            }, 200);
            return;
          }
