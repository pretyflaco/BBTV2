 ...
 * Build version: v19-trace-logout
 * 
 * Provides:
...
export function NostrAuthProvider({ children }) {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      console.log('[useNostrAuth] BUILD VERSION: v23-deferred-nav');
    ...
    if (NostrAuthService.hasPendingChallengeFlow()) {
      ...
      if (nostrReturn?.startsWith('challenge')) {
        ...
        if (flow?.step === 'awaitingSignedChallenge') {
          ...
          // This means either:
          // 1. User just returned from pubkey step but redirect to sign step didn't happen yet
          // 2. User manually navigated back without completing sign step
          // 
          // We should NOT call handleChallengeSignReturn() here - it will fail
          // Instead, we should retry the redirect to Amber for signing
          console.log('[useNostrAuth] checkPendingSignerFlow - Flow is awaitingSignedChallenge but no signed event in URL');
          console.log('[useNostrAuth] checkPendingSignerFlow - Attempting to retry redirect to Amber for signing...');
          result = await NostrAuthService.retrySignChallengeRedirect();
          console.log('[useNostrAuth] checkPendingSignerFlow - retrySignChallengeRedirect result:', JSON.stringify(result));
