...
export function NostrAuthProvider({ children }) {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      logAuth('useNostrAuth', 'checkAuth() ENTRY - instance:', providerInstanceId.current, 'authCheckInitiated:', authCheckInitiated.current, 'challengeFlowHandling:', challengeFlowHandling.current);
    ...
    if (NostrAuthService.hasPendingChallengeFlow()) {
      ...
      if (nostrReturn?.startsWith('challenge')) {
        ...
        if (flow?.step === 'awaitingSignedChallenge') {
          ...
          console.log('[useNostrAuth] checkPendingSignerFlow - Returning needsManualStep2 for user-initiated navigation');
  ...
  const signInWithNostrConnect = useCallback(async (publicKey, options = {}) => {
    ...
    logAuth('useNostrAuth', 'signInWithNostrConnect called, pubkey:', publicKey?.slice(0, 16) + '...');
