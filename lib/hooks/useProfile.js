...
export function ProfileProvider({ children }) {
  ...
  const { isAuthenticated, publicKey, profile: authProfile, refreshProfile: refreshAuthProfile, hasServerSession } = useNostrAuth();
   ...
   */
  const syncNpubCashWalletsToServer = useCallback(async (accounts) => {
    if (!publicKey) return;
    ...
    syncTimerRef.current = setTimeout(async () => {
      try {
        ...
        const npubCashWallets = accounts.filter(a => a.type === 'npub-cash');
        ...
        console.log('[useProfile] Syncing', npubCashWallets.length, 'npub.cash wallets to server...');
        
        const walletsToSync = npubCashWallets.map(wallet => ({
          id: wallet.id,
          label: wallet.label,
   ...
   */
  const syncNpubCashWalletsToServerImmediate = useCallback(async (accounts) => {
    if (!publicKey) return;
    
    try {
      const npubCashWallets = accounts.filter(a => a.type === 'npub-cash');
      
      console.log('[useProfile] IMMEDIATE sync:', npubCashWallets.length, 'npub.cash wallets to server');
   ...
   */
  const syncLnAddressWalletsToServerImmediate = useCallback(async (wallets) => {
    if (!publicKey) return;
    
    try {
      const lnAddressWallets = wallets.filter(w => w.type === 'ln-address');
      
      console.log('[useProfile] IMMEDIATE sync:', lnAddressWallets.length, 'LN Address wallets to server');
   ...
   */
  const syncBlinkApiAccountsToServerImmediate = useCallback(async (accounts) => {
    if (!publicKey) return;
    
    try {
      const apiAccounts = accounts.filter(a => a.type !== 'ln-address' && a.type !== 'npub-cash' && a.apiKey);
      
      console.log('[useProfile] IMMEDIATE sync:', apiAccounts.length, 'Blink API accounts to server');
  ...
  const fetchBlinkDataFromServer = useCallback(async () => {
    if (!publicKey) return { blinkApiAccounts: [], blinkLnAddressWallets: [], npubCashWallets: [] };
    ...
    if (!hasServerSession) {
      console.log('[useProfile] Skipping server fetch - no session yet (hasServerSession:', hasServerSession, ')');
  ...
  const loadProfileData = useCallback(async () => {
    ...
    const serverNpubCashWallets = serverData.npubCashWallets || [];
    
    // Separate local accounts by type
    const localApiKeyAccounts = localAccounts.filter(a => a.type !== 'ln-address' && a.type !== 'npub-cash' && a.apiKey);
    const localLnAddressWallets = localAccounts.filter(a => a.type === 'ln-address');
    const localNpubCashWallets = localAccounts.filter(a => a.type === 'npub-cash');
    ...
    if (serverApiAccounts.length > 0) {
      ...
      mergedAccounts = [...mergedApiAccounts, ...localLnAddressWallets, ...localNpubCashWallets];
    ...
    if (serverLnAddressWallets.length > 0) {
      const currentApiAccounts = mergedAccounts.filter(a => a.type !== 'ln-address' && a.type !== 'npub-cash');
      const currentNpubCashWallets = mergedAccounts.filter(a => a.type === 'npub-cash');
      ...
      mergedAccounts = [...currentApiAccounts, ...mergedLnAddressWallets, ...currentNpubCashWallets];
    ...
    if (serverNpubCashWallets.length > 0) {
      const currentApiAccounts = mergedAccounts.filter(a => a.type !== 'ln-address' && a.type !== 'npub-cash');
      const currentLnAddressWallets = mergedAccounts.filter(a => a.type === 'ln-address');
      const mergedNpubCashWallets = [];
      ...
      for (const serverWallet of serverNpubCashWallets) {
        ...
        if (localWallet) {
          ...
          // Add from server (new wallet from another device)
          console.log('[useProfile] Adding server npub.cash wallet to local:', serverWallet.address || serverWallet.lightningAddress);
          mergedNpubCashWallets.push({
            ...
            createdAt: serverWallet.createdAt ? new Date(serverWallet.createdAt).getTime() : Date.now(),
            lastUsed: serverWallet.lastUsed ? new Date(serverWallet.lastUsed).getTime() : undefined,
            source: 'server'
          });
          needsLocalUpdate = true;
      ...
      for (const localWallet of localNpubCashWallets) {
        if (!serverNpubCashWallets.find(s => s.id === localWallet.id)) {
          mergedNpubCashWallets.push(localWallet);
          needsServerSyncNpubCash = true;
        }
      }
      
      mergedAccounts = [...currentApiAccounts, ...currentLnAddressWallets, ...mergedNpubCashWallets];
    } else if (localNpubCashWallets.length > 0) {
      // No server npub.cash wallets but we have local - sync to server
      console.log('[useProfile] No server npub.cash wallets, syncing', localNpubCashWallets.length, 'local wallets to server');
      needsServerSyncNpubCash = true;
    }
  ...
  }, [isAuthenticated, authProfile, updateState, fetchBlinkDataFromServer, syncBlinkApiAccountsToServer, syncLnAddressWalletsToServer, syncNpubCashWalletsToServer]);
   ...
   */
  const addNpubCashAccount = useCallback(async ({ lightningAddress, label }) => {
    if (!authProfile) throw new Error('Not authenticated');
    ...
    try {
      const wallet = await ProfileStorage.addNpubCashAccount(
        profileId,
        { lightningAddress, label }
      );
   ...
   */
  const removeBlinkAccount = useCallback(async (accountId) => {
    if (!authProfile) throw new Error('Not authenticated');
    
    try {
      // Get the account type before removing
      const account = authProfile.blinkAccounts.find(a => a.id === accountId);
  ...
  }, [authProfile, refreshAuthProfile, updateState, syncBlinkApiAccountsToServerImmediate, syncLnAddressWalletsToServerImmediate, syncNpubCashWalletsToServerImmediate]);
  ...
  const activeBlinkAccount = useMemo(
    () => state.blinkAccounts.find(a => a.isActive && a.type !== 'npub-cash') || null,
    [state.blinkAccounts]
  );
  ...
  const npubCashWallets = useMemo(
    () => state.blinkAccounts.filter(a => a.type === 'npub-cash'),
    [state.blinkAccounts]
  );
  const hasNpubCashWallet = npubCashWallets.length > 0;
  const activeNpubCashWallet = useMemo(
    () => npubCashWallets.find(w => w.isActive) || null,
    [npubCashWallets]
  );
