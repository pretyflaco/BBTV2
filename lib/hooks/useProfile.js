...
export function ProfileProvider({ children }) {
  ...
  /**
   * Fetch LN Address wallets from server
   */
  const fetchLnAddressWalletsFromServer = useCallback(async () => {
    if (!publicKey) return [];
    
    try {
      console.log('[useProfile] Fetching LN Address wallets from server...');
      ...
      console.log('[useProfile] Server returned', data.blinkLnAddressWallets?.length || 0, 'LN Address wallets');
  ...
  /**
   * Load profile data (with server sync for LN Address wallets)
   */
  const loadProfileData = useCallback(async () => {
    ...
    // Fetch LN Address wallets from server for cross-device sync
    const serverLnAddressWallets = await fetchLnAddressWalletsFromServer();
    
    if (serverLnAddressWallets.length > 0) {
      const localLnAddressWallets = localAccounts.filter(a => a.type === 'ln-address');
      const localApiKeyWallets = localAccounts.filter(a => a.type !== 'ln-address');
      
      // Merge server LN Address wallets with local
      const mergedLnAddressWallets = [];
      
      // Add all server wallets, updating with local data if exists
      for (const serverWallet of serverLnAddressWallets) {
        const localWallet = localLnAddressWallets.find(l => l.id === serverWallet.id);
        
        if (localWallet) {
          // Keep local version (may have more recent state)
          ...
          mergedLnAddressWallets.push({
            ...
            isActive: serverWallet.isActive,
      ...
      // Combine with API key wallets
      const mergedAccounts = [...localApiKeyWallets, ...mergedLnAddressWallets];
      ...
      // Sync any local-only wallets to server
      const localOnlyWallets = localLnAddressWallets.filter(
        l => !serverLnAddressWallets.find(s => s.id === l.id)
      );
      if (localOnlyWallets.length > 0) {
        console.log('[useProfile] Syncing', localOnlyWallets.length, 'local-only LN Address wallets to server');
        syncLnAddressWalletsToServer(mergedLnAddressWallets);
      }
    } else if (localAccounts.filter(a => a.type === 'ln-address').length > 0) {
      // No server wallets but we have local - sync to server
      console.log('[useProfile] No server LN Address wallets, syncing local to server');
      syncLnAddressWalletsToServer(localAccounts);
    }
  }, [isAuthenticated, authProfile, updateState, fetchLnAddressWalletsFromServer, syncLnAddressWalletsToServer]);
