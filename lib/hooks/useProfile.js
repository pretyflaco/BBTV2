...
export function ProfileProvider({ children }) {
  ...
  const setActiveBlinkAccount = useCallback((accountId) => {
    ...
    try {
      ProfileStorage.setActiveBlinkAccount(authProfile.id, accountId);
      refreshAuthProfile();
      loadProfileData();
    } catch (error) {
  ...
  const updateBlinkAccount = useCallback(async (accountId, updates) => {
    ...
    try {
      await ProfileStorage.updateBlinkAccount(authProfile.id, accountId, updates);
      refreshAuthProfile();
      loadProfileData();
      updateState({ loading: false });
      return { success: true };
