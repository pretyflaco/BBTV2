...
export function ProfileProvider({ children }) {
  ...
  const removeBlinkAccount = useCallback((accountId) => {
    ...
    try {
      ...
      // Get the updated profile and sync to server to persist the deletion
      const freshProfile = ProfileStorage.getProfileById(authProfile.id);
      if (freshProfile) {
        // Sync the appropriate wallet type to server to persist deletion
      ...
      updateState({
        blinkAccounts: freshProfile?.blinkAccounts || [],
        serverSynced: false // Mark as needing sync
      });
  ...
  }, [authProfile, refreshAuthProfile, updateState, syncBlinkApiAccountsToServer, syncLnAddressWalletsToServer, syncNpubCashWalletsToServer]);
