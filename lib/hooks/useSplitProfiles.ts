 ...
 */
export type RecipientType = 'blink' | 'lightning' | 'onchain';
...
export function useSplitProfiles(): UseSplitProfilesReturn {
  ...
  const [editingSplitProfile, setEditingSplitProfile] = useState<SplitProfile | null>(null);

  // Form state
  const [newSplitProfileLabel, setNewSplitProfileLabel] = useState<string>('');
  const [newSplitProfileRecipients, setNewSplitProfileRecipients] = useState<SplitRecipient[]>([]);
  ...
  // Recipient validation state
  const [recipientValidation, setRecipientValidation] = useState<RecipientValidationState>(
    DEFAULT_RECIPIENT_VALIDATION
  );
