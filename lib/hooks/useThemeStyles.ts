...
export type ThemeType = "dark" | "light" | "blink-classic-dark" | "blink-classic-light"
...
export interface ThemeStylesReturn {
  ...
  getWalletDeleteButtonClasses: () => string
 ...
 * ```
 */
export function useThemeStyles(): ThemeStylesReturn {
  const { theme, darkMode, isBlinkClassic, isBlinkClassicDark, isBlinkClassicLight } =
    useTheme()
  ...
  const styles = useMemo(
    () => ({
      ...
      getWalletCardActiveClasses: (
        accentColor: "amber" | "purple" | "teal" = "amber",
      ): string => {
        switch (theme) {
          case "blink-classic-dark":
          ...
          default:
            ...
            if (accentColor === "purple")
              return "bg-purple-50 dark:bg-purple-900/20 border border-purple-400 dark:border-purple-500 rounded-lg"
            if (accentColor === "teal")
              return "bg-teal-50 dark:bg-teal-900/20 border border-teal-400 dark:border-teal-500 rounded-lg"
      ...
      getWalletIconClasses: (isActive: boolean): string => {
        switch (theme) {
          case "blink-classic-dark":
            return isActive
              ? "bg-blink-classic-bg border border-blink-classic-amber"
              : "bg-transparent border border-blink-classic-border"
          case "blink-classic-light":
            return isActive
              ? "bg-blink-classic-hover-light border border-blink-classic-amber"
              : "bg-transparent border border-blink-classic-border-light"
          default:
            return isActive
              ? "bg-blink-accent/20"
              : darkMode
