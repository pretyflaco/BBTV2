...
export function useTipSettings(): UseTipSettingsReturn {
   ...
   */
  const setTipsEnabledPersistent = useCallback(
   ...
   */
  const setTipPresetsPersistent = useCallback(
   ...
   */
  const setActiveTipProfilePersistent = useCallback(
    (value: TipProfile | null | ((prev: TipProfile | null) => TipProfile | null)) => {
      setActiveTipProfile((prev) => {
        const newValue = typeof value === "function" ? value(prev) : value
        if (isBrowser()) {
          if (newValue) {
            localStorage.setItem(
              STORAGE_KEYS.ACTIVE_TIP_PROFILE,
