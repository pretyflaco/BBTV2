...
function getPool() {
  if (!pool) {
    pool = new Pool({
      connectionString: process.env.DATABASE_URL || 'postgresql://blinkpos:blinkpos_dev_password@localhost:5432/blinkpos',
      max: 20,
...
async function reviewApplication(membershipId, approvedByNpub, approved, rejectionReason) {
  const result = await query(
    `UPDATE community_memberships 
     ...
     WHERE id = $4
     RETURNING *`,
    [approved ? 'approved' : 'rejected', approvedByNpub, rejectionReason, membershipId]
  );
  return result.rows[0];
}
