 ...
 * @param {number} membershipId 
 * @param {string} removedByNpub - The npub of the leader/admin who removed the member
 * @returns {Object}
 */
async function removeMember(membershipId, removedByNpub) {
  const result = await query(
    `UPDATE community_memberships 
     SET status = 'removed', 
         removed_at = NOW(),
     ...
     WHERE id = $1
     RETURNING *`,
    [parseInt(membershipId, 10), removedByNpub]
  );
 ...
 * @param {number} consentId - The consent ID
 * @param {string} communityId - The community ID
 * @param {number} btcBalanceSats - BTC wallet balance in satoshis
 * @param {number} stablesatsBalanceSats - StableSats (USD) wallet balance in satoshis
 * @returns {Object} The inserted/updated snapshot
 */
async function insertBalanceSnapshot(consentId, communityId, btcBalanceSats, stablesatsBalanceSats) {
  const result = await query(
 ...
 * @returns {Object|null}
 */
async function getLatestBalanceSnapshot(consentId) {
  const result = await query(
 ...
 * @param {string} communityId 
 * @param {Date} asOfDate - Optional date (defaults to today)
 * @returns {Object} Bitcoin preference data
 */
async function getCommunityBitcoinPreference(communityId, asOfDate = new Date()) {
  ...
  return {
    ...
    total_btc_sats: parseInt(row.total_btc_sats) || 0,
    total_stablesats_sats: parseInt(row.total_stablesats_sats) || 0,
    total_balance_sats: parseInt(row.total_balance_sats) || 0,
    member_count: parseInt(row.member_count) || 0,
    members_with_balance: parseInt(row.members_with_balance) || 0,
    has_data: (parseInt(row.member_count) || 0) > 0
  };
}
 ...
 * @returns {Array} Array of daily preference data
 */
async function getBitcoinPreferenceHistory(communityId, days = 30) {
  const result = await query(
    `SELECT 
       snapshot_date,
...
module.exports = {
  ...
  getDataCoverage,
  getDateRange,
  // Balance Snapshots & Bitcoin Preference
  insertBalanceSnapshot,
  getLatestBalanceSnapshot,
  getCommunityBitcoinPreference,
  getBitcoinPreferenceHistory,
  // Milestones
