...
// Try to import @noble/secp256k1 for Schnorr signature verification
let secp256k1;
let sha256;

try {
  secp256k1 = require('@noble/secp256k1');
  sha256 = require('@noble/hashes/sha256').sha256;
} catch (e) {
  console.warn('NIP-98: @noble/secp256k1 not available, using fallback verification');
}
...
async function verifySchnorrSignature(signature, message, publicKey) {
  ...
  try {
    ...
    if (secp256k1.schnorr && secp256k1.schnorr.verify) {
      return await secp256k1.schnorr.verify(sigBytes, msgBytes, pubBytes);
    }
    
    // Fallback for older versions
    if (secp256k1.verify) {
      return secp256k1.verify(sigBytes, msgBytes, pubBytes);
    }
    
    console.error('NIP-98: No compatible verification method found');
    return false;
  } catch (error) {
