...
async function loadModules() {
  ...
  moduleLoadPromise = (async () => {
    try {
      ...
      const secpModule = await import('@noble/secp256k1');
      const hashesModule = await import('@noble/hashes/sha256');
      
      // ESM modules may export differently - check all possibilities
      // @noble/secp256k1 v2.x exports: schnorr, getPublicKey, sign, verify, etc.
      console.log('NIP-98: secp256k1 module exports:', Object.keys(secpModule));
      console.log('NIP-98: hashes module exports:', Object.keys(hashesModule));
      
      // The module might have schnorr directly, or in default, or be the default itself
      if (secpModule.schnorr) {
        secp256k1 = secpModule;
        console.log('NIP-98: Using secpModule directly (has schnorr)');
      } else if (secpModule.default?.schnorr) {
        secp256k1 = secpModule.default;
        console.log('NIP-98: Using secpModule.default (has schnorr)');
      } else {
        // Try to construct the expected interface
        secp256k1 = secpModule.default || secpModule;
        console.log('NIP-98: Using fallback secp256k1 assignment');
      }
      
      sha256Fn = hashesModule.sha256 || hashesModule.default?.sha256;
      modulesLoaded = true;
      
      console.log('NIP-98: Loaded @noble/secp256k1 and @noble/hashes successfully');
      console.log('NIP-98: secp256k1 object keys:', secp256k1 ? Object.keys(secp256k1) : 'null');
      console.log('NIP-98: schnorr available:', secp256k1?.schnorr ? 'yes' : 'no');
      
      return { secp256k1, sha256: sha256Fn };
    } catch (e) {
      console.error('NIP-98: Failed to load @noble packages:', e);
