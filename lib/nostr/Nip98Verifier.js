 ...
 * Uses @noble/curves for BIP-340 Schnorr signatures (required for Nostr)
 */
async function loadModules() {
  ...
  moduleLoadPromise = (async () => {
    try {
      // Use @noble/curves/secp256k1 which has BIP-340 Schnorr signatures
      // @noble/secp256k1 v2.x moved Schnorr to @noble/curves
      const curvesModule = await import('@noble/curves/secp256k1');
      const hashesModule = await import('@noble/hashes/sha256');
      
      console.log('NIP-98: curves/secp256k1 exports:', Object.keys(curvesModule));
      console.log('NIP-98: Has schnorr:', !!curvesModule.schnorr);
      
      secp256k1 = curvesModule;
      sha256Fn = hashesModule.sha256;
      modulesLoaded = true;
      
      if (curvesModule.schnorr) {
        console.log('NIP-98: ✓ Loaded @noble/curves with Schnorr support');
        console.log('NIP-98: schnorr.verify available:', typeof curvesModule.schnorr.verify);
      } else {
 ...
 * @param {string} signature - Signature as hex string (128 chars = 64 bytes)
 * @param {string} message - Message hash as hex string (event ID)
 * @param {string} publicKey - Public key as hex string (64 chars = 32 bytes)
...
async function verifySchnorrSignature(signature, message, publicKey) {
  ...
  try {
    ...
    console.log('NIP-98: Verifying signature...');
    console.log('NIP-98: Signature length:', sigBytes.length, 'bytes');
    console.log('NIP-98: Message length:', msgBytes.length, 'bytes');
    console.log('NIP-98: Pubkey length:', pubBytes.length, 'bytes');
    
    // @noble/curves/secp256k1 schnorr.verify(sig, msg, pubkey)
    const result = secp256k1.schnorr.verify(sigBytes, msgBytes, pubBytes);
