...
class NostrAuthService {
  ...
  static async handleChallengeFlowReturn() {
    ...
    if (flow.step === 'awaitingPubkey') {
      ...
      if (!pubkey) {
        localStorage.removeItem(this.CHALLENGE_STORAGE_KEY);
        return { success: false, error: 'Could not get pubkey from signer' };
      }
      ...
      console.log('[NostrAuthService] Requesting challenge signature from external signer...');
      window.location.href = nostrSignerURL;
  ...
  static async handleChallengeSignReturn() {
    ...
    // Try to get signed event from clipboard or URL
    let signedEventJson = urlParams.get('result') || urlParams.get('event');
  ...
  static hasPendingChallengeFlow() {
    ...
    if (nostrReturn === 'challenge' || nostrReturn === 'signed') {
      return true;
    }
