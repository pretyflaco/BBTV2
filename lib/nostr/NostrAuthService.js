...
class NostrAuthService {
  ...
  static signInWithNostrConnect(publicKey) {
    try {
      ...
      console.log('[NostrAuthService] Signed in with Nostr Connect, pubkey:', normalizedPublicKey.slice(0, 16) + '...');
   ...
   * 
   * v65: Fix to check which service is actually connected, not just the feature flag
  ...
  static async signEventWithNostrConnect(event) {
    ...
    console.log('[NostrAuthService] v65: Checking connected services - NDK:', ndkConnected, 'Legacy:', legacyConnected);
    
    // v65: Use whichever service is actually connected
    // Priority: If both connected, prefer NDK when USE_NDK is true
    if (ndkConnected && (USE_NDK || !legacyConnected)) {
      console.log('[NostrAuthService] v65: Using NDK service for signing');
      ...
      console.log('[NostrAuthService] v65: Using legacy nostr-tools service for signing');
