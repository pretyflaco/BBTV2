...
async function navigateToSignerUrl(url, context = 'unknown') {
  ...
  console.log('[NostrAuthService] Scheduling deferred navigation via setTimeout...');
  
  return new Promise((resolve) => {
    // Schedule for next event loop tick
    setTimeout(() => {
      console.log('[NostrAuthService] Deferred navigation executing now...');
      try {
        // Try location.replace first - this is more "forceful" and replaces the current history entry
        console.log('[NostrAuthService] Trying window.location.replace()...');
        window.location.replace(url);
        console.log('[NostrAuthService] location.replace() called');
        resolve(true);
      } catch (e) {
        console.warn('[NostrAuthService] location.replace() failed:', e);
        ...
        try {
          ...
          console.error('[NostrAuthService] location.href assignment failed:', e2);
...
class NostrAuthService {
  ...
  static async signInWithExternalSignerChallenge() {
    try {
      ...
      // CRITICAL FIX (v20/v21): Don't overwrite an existing pending flow!
      ...
      if (pendingFlow && pendingFlow.step === 'awaitingPubkey') {
        ...
        if (flowAge < maxFlowAge) {
          console.log('[NostrAuthService] Challenge flow already pending (age: ' + Math.round(flowAge/1000) + 's), reusing existing challenge');
          ...
          console.log('[NostrAuthService] Retrying signer redirect with existing challenge...');
          ...
          console.log('[NostrAuthService] Existing flow expired (age: ' + Math.round(flowAge/1000) + 's), starting fresh');
  ...
  static clearPendingChallengeFlow() {
    console.log('[NostrAuthService] clearPendingChallengeFlow called');
    localStorage.removeItem(this.CHALLENGE_STORAGE_KEY);
  }
  ...
  static async retrySignChallengeRedirect() {
    console.log('[NostrAuthService] retrySignChallengeRedirect called');
    
    const flow = this.getPendingChallengeFlow();
    console.log('[NostrAuthService] retrySignChallengeRedirect - flow:', JSON.stringify(flow));
    
    if (!flow || flow.step !== 'awaitingSignedChallenge') {
      console.log('[NostrAuthService] retrySignChallengeRedirect - Invalid flow state');
      return { success: false, error: 'Invalid flow state for retry' };
    }
    
    if (!flow.pubkey) {
      console.log('[NostrAuthService] retrySignChallengeRedirect - No pubkey in flow');
      localStorage.removeItem(this.CHALLENGE_STORAGE_KEY);
      return { success: false, error: 'No pubkey in flow, please restart sign-in' };
    }
    
    if (!flow.challenge) {
      console.log('[NostrAuthService] retrySignChallengeRedirect - No challenge in flow');
      localStorage.removeItem(this.CHALLENGE_STORAGE_KEY);
    ...
    if (flowAge > 5 * 60 * 1000) {
      console.log('[NostrAuthService] retrySignChallengeRedirect - Flow too old:', flowAge, 'ms');
      localStorage.removeItem(this.CHALLENGE_STORAGE_KEY);
      return { success: false, error: 'Challenge expired, please restart sign-in' };
    }
    
    console.log('[NostrAuthService] retrySignChallengeRedirect - Rebuilding sign request...');
    console.log('[NostrAuthService] retrySignChallengeRedirect - pubkey:', flow.pubkey.substring(0, 16) + '...');
    console.log('[NostrAuthService] retrySignChallengeRedirect - challenge:', flow.challenge.substring(0, 30) + '...');
    ...
    console.log('[NostrAuthService] retrySignChallengeRedirect - Challenge event:', JSON.stringify(challengeEvent).substring(0, 100) + '...');
    ...
    console.log('[NostrAuthService] retrySignChallengeRedirect - Redirecting to signer for challenge signature...');
    console.log('[NostrAuthService] retrySignChallengeRedirect - Callback URL:', signCallbackUrl.toString());
    
    // Navigate to signer app
    const navSuccess = await navigateToSignerUrl(nostrSignerURL, 'retrySignChallengeRedirect');
    
    if (!navSuccess) {
      console.error('[NostrAuthService] retrySignChallengeRedirect - Failed to navigate to signer');
      return { 
        success: false, 
    ...
    console.log('[NostrAuthService] retrySignChallengeRedirect - Navigation initiated, returning pending');
    return { success: true, pending: true, method: 'externalSigner' };
  }
  ...
  // ============================================================================
  // v25: URL Builder Methods for Anchor Links
  // These return the nostrsigner: URL strings so UI can render actual <a> elements
  // instead of relying on programmatic navigation which fails on some devices
  // ============================================================================

  /**
   * v25: Build the nostrsigner: URL for Step 1 (get public key)
   * This creates a new challenge flow and returns the URL to navigate to.
   * Call this when rendering the "Sign in with Amber" anchor link.
   * 
   * @returns {Promise<{url: string, error?: string}>}
   */
  static async buildGetPubkeyUrl() {
    console.log('[NostrAuthService] v25: buildGetPubkeyUrl called');
    ...
    try {
      ...
      const challengeResponse = await fetch('/api/auth/challenge', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ method: 'externalSigner' }),
        credentials: 'include'
      });

      if (!challengeResponse.ok) {
        const errorData = await challengeResponse.json().catch(() => ({}));
        console.error('[NostrAuthService] v25: Failed to get challenge:', errorData);
        return { url: '', error: errorData.message || 'Failed to get authentication challenge' };
      }

      const { challenge } = await challengeResponse.json();
      console.log('[NostrAuthService] v25: Got challenge:', challenge?.substring(0, 30) + '...');

      // Store challenge flow data
      const challengeFlow = {
        challenge,
        step: 'awaitingPubkey',
        timestamp: Date.now()
      };
      localStorage.setItem(this.CHALLENGE_STORAGE_KEY, JSON.stringify(challengeFlow));

      // Build callback URL for pubkey return
      const callbackUrl = new URL(window.location.href);
      // Clean any existing signer params
      callbackUrl.searchParams.delete(SIGNER_RETURN_PARAM);
      callbackUrl.searchParams.delete('pubkey');
      callbackUrl.searchParams.delete('result');
      callbackUrl.searchParams.set(SIGNER_RETURN_PARAM, 'challenge');
      
      const returnUrl = callbackUrl.toString();
      console.log('[NostrAuthService] v25: Callback URL:', returnUrl);
      ...
      console.log('[NostrAuthService] v25: Built get_public_key URL');
      return { url: nostrSignerURL };
    } catch (error) {
      console.error('[NostrAuthService] v25: Error building pubkey URL:', error);
      return { url: '', error: error.message || 'Failed to prepare authentication' };
    }
  }

  /**
   * v25: Build the nostrsigner: URL for Step 2 (sign challenge)
   * This reads the existing challenge flow and builds the sign event URL.
   * Call this when rendering the "Continue to Amber" anchor link.
   * 
   * @returns {{url: string, error?: string}}
   */
  static buildSignChallengeUrl() {
    console.log('[NostrAuthService] v25: buildSignChallengeUrl called');
    
    const flow = this.getPendingChallengeFlow();
    
    if (!flow || flow.step !== 'awaitingSignedChallenge') {
      console.log('[NostrAuthService] v25: Invalid flow state for sign URL');
      return { url: '', error: 'Invalid flow state - please restart sign-in' };
    }
    
    if (!flow.pubkey) {
      console.log('[NostrAuthService] v25: No pubkey in flow');
      return { url: '', error: 'No pubkey in flow - please restart sign-in' };
    }
    
    if (!flow.challenge) {
      console.log('[NostrAuthService] v25: No challenge in flow');
      return { url: '', error: 'No challenge in flow - please restart sign-in' };
    }
    
    // Check if flow is too old (more than 5 minutes)
    const flowAge = Date.now() - (flow.timestamp || 0);
    if (flowAge > 5 * 60 * 1000) {
      console.log('[NostrAuthService] v25: Flow expired, age:', flowAge, 'ms');
      localStorage.removeItem(this.CHALLENGE_STORAGE_KEY);
      return { url: '', error: 'Challenge expired - please restart sign-in' };
    }
    ...
    console.log('[NostrAuthService] v25: Challenge event built for pubkey:', flow.pubkey.substring(0, 16) + '...');
    ...
    console.log('[NostrAuthService] v25: Built sign_event URL');
    return { url: nostrSignerURL };
  }
}
