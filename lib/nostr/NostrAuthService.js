...
class NostrAuthService {
  ...
  static async signInWithExternalSigner(callbackUrl) {
    try {
      ...
      // Remove any existing nostr_return param
      currentUrl.searchParams.delete(SIGNER_RETURN_PARAM);
      // Add return indicator so we know this is a return from signer
      currentUrl.searchParams.set(SIGNER_RETURN_PARAM, '1');
      const returnUrl = callbackUrl || currentUrl.toString();
      ...
      // Format: nostrsigner:<type>?<params>
      // For get_public_key, we ask the signer to return via callback or clipboard
      const nostrSignerURL = `nostrsigner:get_public_key?callbackUrl=${encodeURIComponent(returnUrl)}&compressionType=none&returnType=signature`;
  ...
  static async handleExternalSignerReturn() {
    try {
      ...
      const urlPubkey = urlParams.get('pubkey') || urlParams.get('result') || urlParams.get('signature');
      
      if (urlPubkey) {
        console.log('Got public key from URL parameters');
