...
class NostrAuthService {
  ...
  static async signInWithExternalSignerChallenge() {
    try {
      ...
      // CRITICAL FIX (v20): Don't overwrite an existing pending flow!
      ...
      if (pendingFlow && pendingFlow.step === 'awaitingPubkey') {
        ...
        if (flowAge < maxFlowAge) {
          console.log('[NostrAuthService] Challenge flow already pending (age: ' + Math.round(flowAge/1000) + 's), not starting new one');
