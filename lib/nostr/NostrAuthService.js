...
class NostrAuthService {
  ...
  static async signInWithExternalSigner(callbackUrl) {
    try {
      ...
      // Format: nostrsigner:?type=get_public_key&callbackUrl=...
      // Amber parses: type from URL params, callbackUrl for return
      // returnType=signature means return just the pubkey, not a full event
      ...
      const nostrSignerURL = 'nostrsigner:?' + params.toString();
      
      console.log('Opening external signer with URL:', nostrSignerURL);
      console.log('Callback URL:', returnUrl);

      // Navigate to signer - we don't wait for visibility change
      // The user will be returned via the callback URL
  ...
  static async handleExternalSignerReturn() {
    try {
      ...
      // Method 1b: Also check if pubkey was appended to the URL without proper encoding
      // Amber does: callbackUrl + Uri.encode(value), so if callbackUrl ended with "&pubkey="
      // the result is "&pubkey=abc123..." which urlParams should parse
      // But let's also check the raw URL just in case
      if (!urlPubkey) {
        const fullUrl = window.location.href;
        // Look for a 64-char hex string at the end of the URL
        const hexMatch = fullUrl.match(/[?&]pubkey=([a-f0-9]{64})/i);
        if (hexMatch) {
          urlPubkey = hexMatch[1];
        }
      }
      ...
      if (!publicKey) {
        return {
          success: false,
          error: 'Could not retrieve public key. Please ensure you approved the request in Amber and try again.'
        };
      }
  ...
  static hasPendingExternalSignerFlow() {
    try {
      ...
      if (typeof window !== 'undefined') {
        const urlParams = new URLSearchParams(window.location.search);
        if (urlParams.has(SIGNER_RETURN_PARAM)) {
          return true;
        }
      }
