...
class NostrAuthService {
  ...
  // Guard flag to prevent multiple redirects to external signer during React re-renders
  static _redirectingToSigner = false;
  
  // Guard flag to prevent multiple processing of the same return
  static _processingReturn = false;

  /**
   * Reset the redirect guard (call on successful auth or when starting new flow)
   */
  static resetRedirectGuard() {
    this._redirectingToSigner = false;
  ...
  static async signInWithExternalSigner(callbackUrl) {
    try {
      ...
      // Try to open the signer app
      // On both iOS and Android, window.location.href should trigger the URL scheme
  ...
  static async signInWithExternalSignerChallenge() {
    try {
      ...
      // Reset redirect guard at the start of a new sign-in flow
      this.resetRedirectGuard();
      ...
      // Reset redirect guard and then set it (starting fresh flow)
      this._redirectingToSigner = true;
   ...
   * Used when user wants to restart the sign-in flow
   */
  static clearPendingChallengeFlow() {
    try {
      localStorage.removeItem(this.CHALLENGE_STORAGE_KEY);
      this.resetRedirectGuard(); // Also reset redirect guard
      console.log('[NostrAuthService] Cleared pending challenge flow');
    } catch (e) {
      console.warn('[NostrAuthService] Failed to clear pending challenge flow:', e);
    }
  }
   ...
   * @param {string|null} capturedNostrReturn - Pre-captured nostr_return value (to avoid race conditions)
   * @returns {Promise<AuthResult>}
   */
  static async handleChallengeFlowReturn(capturedNostrReturn = null) {
    // Guard against multiple concurrent calls (React re-renders)
    if (this._processingReturn) {
      console.log('[NostrAuthService] handleChallengeFlowReturn: Already processing, returning pending');
      return { success: true, pending: true, method: 'externalSigner' };
    }
    this._processingReturn = true;
    
    // Use captured value if provided (avoids race condition where URL gets cleaned by concurrent calls)
    const urlParams = new URLSearchParams(window.location.search);
    const nostrReturn = capturedNostrReturn || urlParams.get(SIGNER_RETURN_PARAM);
    
    console.log('[NostrAuthService] handleChallengeFlowReturn called');
    console.log('[NostrAuthService] Using captured nostr_return:', capturedNostrReturn ? 'yes' : 'no (from URL)');
    console.log('[NostrAuthService] nostr_return value:', nostrReturn?.substring(0, 30) || 'none');
    ...
    if (flow.step === 'awaitingPubkey') {
      ...
      // Guard against multiple redirects (React re-renders can cause this)
      if (this._redirectingToSigner) {
        console.log('[NostrAuthService] Redirect already in progress, skipping duplicate redirect');
        return { success: true, pending: true, method: 'externalSigner' };
      }
      ...
      // Set flag BEFORE redirect to prevent race conditions
      this._redirectingToSigner = true;
   ...
   * @returns {Promise<AuthResult>}
   */
  static async handleChallengeSignReturn(capturedNostrReturn = null) {
    // Guard against multiple concurrent calls (React re-renders)
    if (this._processingReturn) {
      console.log('[NostrAuthService] handleChallengeSignReturn: Already processing, returning pending');
      return { success: true, pending: true, method: 'externalSigner' };
    }
    this._processingReturn = true;
    ...
    console.log('[NostrAuthService] handleChallengeSignReturn called');
    console.log('[NostrAuthService] Using captured nostr_return:', capturedNostrReturn ? 'yes' : 'no (from URL)');
    console.log('[NostrAuthService] nostr_return value:', nostrReturn?.substring(0, 30) || 'none');
    ...
    // Clean URL (do this early to prevent duplicate processing)
    const cleanUrl = new URL(window.location.href);
    cleanUrl.searchParams.delete(SIGNER_RETURN_PARAM);
    cleanUrl.searchParams.delete('result');
    ...
    // Try to get signed event from multiple sources:
    // 1. Amber's concatenated format: "signed{json-event}"
    // 2. Standard URL params: ?result= or ?event=
    // 3. Clipboard fallback (for when URL params are lost due to Android intent handling)
    let signedEventJson = null;
    
    // First check if we have nostr_return with signed event
    if (nostrReturn?.startsWith('signed') && nostrReturn.length > 6) {
    ...
    if (!signedEventJson) {
      ...
      if (signedEventJson) {
        console.log('[NostrAuthService] Got signed event from URL params');
      }
    }
    
    // Try clipboard as fallback (handles case where Android loses URL params)
    if (!signedEventJson) {
      console.log('[NostrAuthService] No signed event in URL, trying clipboard...');
      signedEventJson = await this.getRawClipboardText(
        'Please paste the signed event from your signer app:'
      );
      if (signedEventJson) {
        console.log('[NostrAuthService] Got signed event from clipboard');
      }
    }
    ...
    try {
      ...
      // Clear flow data and reset redirect guard
      localStorage.removeItem(this.CHALLENGE_STORAGE_KEY);
      this.resetRedirectGuard();
