...
class NostrConnectService {
   ...
   * @returns {Promise<ConnectionResult>}
   */
  static async connectWithBunkerURL(bunkerUrl, maxRetries = 3, forceNewClientKey = false, onAuthUrl = null) {
    try {
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          // Also add pool event listener to debug what nsec.app sends back
          const signerParams = {
            onauth: (authUrl) => {
              ...
              if (onAuthUrl) {
                ...
                if (!authWindow) {
                  ...
                  // If popup is blocked, open in same tab as fallback
                  // The user will need to return manually
          ...
          console.log('[NostrConnect] signerParams.onauth defined:', !!signerParams.onauth);
          
          // DEBUGGING v36: Watch raw responses from nsec.app to understand why onauth isn't triggered
          const debugPool = new SimplePool();
          const signerPubkey = bunkerPointer.pubkey;
          console.log('[NostrConnect] Setting up debug subscription for signer pubkey:', signerPubkey.slice(0, 16) + '...');
          console.log('[NostrConnect] Watching relay:', bunkerPointer.relays[0]);
          console.log('[NostrConnect] For client pubkey:', clientPubkey.slice(0, 16) + '...');
          
          let debugSubClosed = false;
          const debugSub = debugPool.subscribeMany(
            bunkerPointer.relays,
            [{ kinds: [24133], authors: [signerPubkey], '#p': [clientPubkey] }],
            {
              onevent: (event) => {
                console.log('[NostrConnect] *** DEBUG: RAW EVENT FROM SIGNER ***');
                console.log('[NostrConnect] Event ID:', event.id?.slice(0, 16) + '...');
                console.log('[NostrConnect] Event kind:', event.kind);
                console.log('[NostrConnect] Event created_at:', event.created_at);
                console.log('[NostrConnect] Event content (first 100 chars):', event.content?.slice(0, 100));
                // Note: We can't easily decrypt here without conversation key, but logging raw helps
              },
              oneose: () => {
                console.log('[NostrConnect] Debug subscription reached EOSE');
              }
            }
          );
          
          // Clean up debug subscription after timeout
          setTimeout(() => {
            if (!debugSubClosed) {
              debugSubClosed = true;
              debugSub.close();
              console.log('[NostrConnect] Debug subscription closed after timeout');
            }
          }, 35000);
          ...
          // If we got "invalid secret" but auth_url was received, that's actually the auth flow
          // The user needs to approve in the popup, then retry
          if (errorMessage.includes('invalid secret') && authUrlOpened) {
            console.log('[NostrConnect] Auth URL was opened, waiting for user approval...');
            // Return a special status indicating user needs to approve
            authUrlCallback = null;
            return { 
              success: false, 
              error: 'Please approve the connection in nsec.app, then try again.',
              needsApproval: true,
              authUrlOpened: true
            };
          }
