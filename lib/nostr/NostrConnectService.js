...
class NostrConnectService {
   ...
   * @returns {Promise<ConnectionResult>}
   */
  static async connectWithBunkerURL(bunkerUrl, maxRetries = 3) {
    try {
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          console.log('[NostrConnect] Calling signer.connect()...');
          ...
          lastError = error;
          console.warn(`[NostrConnect] Attempt ${attempt} failed:`, error.message || 'no message');
          console.warn(`[NostrConnect] Error type:`, error.constructor?.name);
