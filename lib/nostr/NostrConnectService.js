 ...
 * - Remove retry logic for bunker URL secrets (they are single-use)
 * - Better error messaging for expired/invalid secrets
 * - The secret in bunker:// URLs is consumed on first use
 * 
  ...
  static async connectWithBunkerURL(bunkerUrl, maxRetries = 1, forceNewClientKey = false, onAuthUrl = null) {
    try {
      ...
      try {
        ...
        // v47: Handle "invalid secret" error specifically
        // This means the secret was already used or expired - user MUST generate a new bunker URL
        if (errorMessage.includes('invalid secret')) {
          console.log('[NostrConnect] v47: Got "invalid secret" error');
          
          // If auth URL was opened, user needs to approve
          if (authUrlOpened) {
            console.log('[NostrConnect] Auth URL was opened, waiting for user approval...');
            authUrlCallback = null;
            this.connectionState = 'disconnected';
            this.signer = null;
            return { 
              success: false, 
              error: 'Please approve the connection in nsec.app, then try again with a NEW bunker URL.',
              needsApproval: true,
              authUrlOpened: true,
              secretExpired: true
            };
          ...
          // Secret is expired/consumed - user needs a new bunker URL
          console.log('[NostrConnect] v47: Secret is expired or already used');
          authUrlCallback = null;
          this.connectionState = 'disconnected';
          this.signer = null;
          return { 
