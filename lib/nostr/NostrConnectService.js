...
class NostrConnectService {
  ...
  static getPool() {
    if (!this.pool) {
      console.log('[NostrConnect] Creating new shared SimplePool');
  ...
  static closePool() {
    if (this.pool) {
      console.log('[NostrConnect] Closing shared SimplePool');
      try {
        this.pool.close([]);
      } catch (e) {
   ...
   * 
   * v47: IMPORTANT - Bunker URL secrets are SINGLE-USE!
   * - nsec.app consumes the secret on first connect attempt
   * - Retrying with the same bunker URL will ALWAYS fail with "invalid secret"
   * - If connection fails, user MUST generate a new bunker URL in nsec.app
   * - We use a shared SimplePool per nostr-tools recommendations
   * 
  ...
  static async connectWithBunkerURL(bunkerUrl, maxRetries = 1, forceNewClientKey = false, onAuthUrl = null) {
    try {
      console.log('[NostrConnect] v47: Parsing bunker URL...');
      ...
      console.log('[NostrConnect] maxRetries:', maxRetries, '(note: secrets are single-use)');
      ...
      // Get shared pool per nostr-tools recommendations
      const pool = this.getPool();
      console.log('[NostrConnect] v47: Using shared SimplePool');
      
      // Create signer with onauth callback for nsec.app approval flow
      const signerParams = {
        pool, // Use shared pool
        onauth: (authUrl) => {
          console.log('[NostrConnect] *** ONAUTH CALLBACK TRIGGERED ***');
          console.log('[NostrConnect] Received auth_url from remote signer:', authUrl);
          authUrlOpened = true;
          
          if (onAuthUrl) {
            // Let the UI component handle the auth URL
            console.log('[NostrConnect] Calling UI onAuthUrl callback...');
            onAuthUrl(authUrl);
          } else if (authUrlCallback) {
            // Fallback to stored callback
            console.log('[NostrConnect] Calling stored authUrlCallback...');
            authUrlCallback(authUrl);
          } else {
            // Default: open in new window/tab
            console.log('[NostrConnect] Opening auth URL in new window...');
      ...
      // v47: Log the exact bunker pointer being used
      console.log('[NostrConnect] v47: Creating BunkerSigner with:');
      console.log('[NostrConnect] v47:   bunkerPointer.pubkey:', bunkerPointer.pubkey);
      console.log('[NostrConnect] v47:   bunkerPointer.secret:', bunkerPointer.secret);
      console.log('[NostrConnect] v47:   bunkerPointer.relays:', JSON.stringify(bunkerPointer.relays));
      console.log('[NostrConnect] v47:   clientSecretKey length:', clientSecretKey?.length);
      console.log('[NostrConnect] v47:   clientPubkey:', clientPubkey);
      console.log('[NostrConnect] v47:   using shared pool:', !!signerParams.pool);
      
      this.signer = BunkerSigner.fromBunker(clientSecretKey, bunkerPointer, signerParams);
      
      console.log('[NostrConnect] v47: BunkerSigner created, signer.bp:', JSON.stringify({
      ...
      try {
        ...
        console.warn(`[NostrConnect] Connection failed:`, errorMessage);
        console.warn(`[NostrConnect] Error type:`, typeof error);
        console.warn(`[NostrConnect] Raw error value:`, error);
        console.warn(`[NostrConnect] authUrlOpened at error time:`, authUrlOpened);
        
        // v48: Handle "invalid secret" error
        // IMPORTANT: nsec.app sends "invalid secret" BEFORE the auth_url callback fires
        // We need to wait a moment to see if onauth callback fires
        if (errorMessage.includes('invalid secret')) {
          console.log('[NostrConnect] v48: Got "invalid secret" error, waiting for potential auth_url...');
          
          // Wait up to 2 seconds for the onauth callback to potentially fire
          // The auth_url event often arrives shortly after the "invalid secret" error
          for (let i = 0; i < 20; i++) {
            await new Promise(resolve => setTimeout(resolve, 100));
            if (authUrlOpened) {
              console.log('[NostrConnect] v48: Auth URL was opened during wait!');
              break;
            }
          }
          
          console.log('[NostrConnect] v48: After wait, authUrlOpened:', authUrlOpened);
          
          // If auth URL was opened, user needs to approve then retry with SAME bunker URL
          if (authUrlOpened) {
            console.log('[NostrConnect] v48: Auth URL was opened, user needs to approve...');
            // Don't clear the signer - keep the connection open
            // The user will approve, then we'll retry
            authUrlCallback = null;
            this.connectionState = 'disconnected';
            // Keep signer reference for potential reconnect
            return { 
              success: false, 
              error: 'Please approve the connection in nsec.app, then click "Continue" to complete sign-in.',
              needsApproval: true,
              authUrlOpened: true,
              // NOT secretExpired - the secret is valid, just needs approval
              canRetryWithSameUrl: true
            };
          }
          
          // No auth URL - secret is truly expired/invalid
          console.log('[NostrConnect] v48: No auth URL received - secret is expired or already used');
          authUrlCallback = null;
          this.connectionState = 'disconnected';
          this.signer = null;
          return { 
            success: false, 
            error: 'This bunker URL has expired or was already used. Please generate a NEW bunker URL in nsec.app and try again.',
            secretExpired: true,
            needsNewBunkerUrl: true
          };
        }
  ...
  static async restoreSession() {
    ...
    try {
      ...
      // v47: Use shared pool for session restore
      const pool = this.getPool();
      console.log('[NostrConnect] v47: Using shared SimplePool for session restore');
