...
class NostrConnectServiceNDK {
  ...
  static async connect(bunkerInput, options = {}) {
    ...
    try {
      ...
      // v60: Implement retry logic - on "invalid secret", retry WITHOUT the secret
      //      If the client was approved, it should be able to connect with just the pubkey
      console.log('[NDK-NIP46] v60: Waiting for signer to be ready (with retry logic)...');
      ...
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
          console.log(`[NDK-NIP46] v60: Connection attempt ${attempt}/${MAX_RETRIES}...`);
          ...
          if (user && user.pubkey) {
            console.log('[NDK-NIP46] v60: Signer ready! User pubkey:', user.pubkey.slice(0, 16) + '...');
          ...
          console.log(`[NDK-NIP46] v60: Attempt ${attempt} failed:`, errorMsg);
          ...
          if (errorMsg.includes('invalid secret') || errorMsg.includes('secret')) {
            console.log('[NDK-NIP46] v60: Got "invalid secret" - nsec.app needs approval, will retry...');
            this.connectionState = 'awaiting_approval';
            onStatusChange?.('awaiting_approval');
            
            if (attempt < MAX_RETRIES) {
              // Wait before retrying - give user time to approve in nsec.app
              console.log(`[NDK-NIP46] v60: Waiting ${RETRY_DELAY_MS}ms before retry...`);
              await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS));
              
              // v60: On retries after "invalid secret", try connecting WITHOUT the secret
              ...
              console.log('[NDK-NIP46] v60: Re-creating signer for retry (without secret)...');
              
              // Use just the bunker pubkey, not the full URL with secret
              const retryTarget = this.currentBunkerPubkey || targetPubkey || bunkerInput;
              console.log('[NDK-NIP46] v60: Retry target:', typeof retryTarget === 'string' && retryTarget.length === 64 ? retryTarget.slice(0, 16) + '...' : 'bunker URL');
              
              this.signer = new NDKNip46Signer(this.bunkerNDK, retryTarget, localSigner);
              this.signer.on('authUrl', (url) => {
