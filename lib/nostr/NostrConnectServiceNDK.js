...
class NostrConnectServiceNDK {
  ...
  static async connect(bunkerInput, options = {}) {
    ...
    try {
      ...
      // v52: Use the static bunker() method which properly handles bunker:// URLs including the secret
      // This is the recommended way per NDK docs
      console.log('[NDK-NIP46] Creating NDKNip46Signer using bunker() method...');
      
      // For bunker:// URLs, use the static bunker() method
      // For NIP-05, use the constructor with target pubkey
      if (parsed.type === 'bunker') {
        // Get local signer's nsec for the bunker method
        const localNsec = localSigner.privateKey ? 
          nip19.nsecEncode(hexToBytes(localSigner.privateKey)) : 
          undefined;
        
        console.log('[NDK-NIP46] Using bunker URL:', bunkerInput.slice(0, 50) + '...');
        console.log('[NDK-NIP46] Secret included:', parsed.secret ? 'yes' : 'no');
        
        this.signer = NDKNip46Signer.bunker(this.bunkerNDK, bunkerInput, localNsec);
      } else {
      ...
      console.log('[NDK-NIP46] Waiting for signer to be ready...');
      ...
      console.log('[NDK-NIP46] Signer ready! User pubkey:', user.pubkey.slice(0, 16) + '...');
