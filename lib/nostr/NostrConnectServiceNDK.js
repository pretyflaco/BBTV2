...
class NostrConnectServiceNDK {
  ...
  static async connect(bunkerInput, options = {}) {
    ...
    try {
      ...
      // v59: Implement retry logic for "invalid secret" - nsec.app sends this while waiting for approval
      console.log('[NDK-NIP46] v59: Waiting for signer to be ready (with retry logic)...');
      ...
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
          console.log(`[NDK-NIP46] v59: Connection attempt ${attempt}/${MAX_RETRIES}...`);
          ...
          if (user && user.pubkey) {
            console.log('[NDK-NIP46] v59: Signer ready! User pubkey:', user.pubkey.slice(0, 16) + '...');
          ...
          console.log(`[NDK-NIP46] v59: Attempt ${attempt} failed:`, errorMsg);
          ...
          if (errorMsg.includes('invalid secret') || errorMsg.includes('secret')) {
            console.log('[NDK-NIP46] v59: Got "invalid secret" - nsec.app needs approval, will retry...');
            this.connectionState = 'awaiting_approval';
            onStatusChange?.('awaiting_approval');
            
            if (attempt < MAX_RETRIES) {
              // Wait before retrying - give user time to approve in nsec.app
              console.log(`[NDK-NIP46] v59: Waiting ${RETRY_DELAY_MS}ms before retry...`);
              await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS));
              
              // Re-create the signer for retry (NDK may have closed the connection)
              console.log('[NDK-NIP46] v59: Re-creating signer for retry...');
              this.signer = new NDKNip46Signer(this.bunkerNDK, bunkerInput, localSigner);
              this.signer.on('authUrl', (url) => {
