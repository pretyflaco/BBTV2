...
class NostrConnectServiceNDK {
  ...
  static async connect(bunkerInput, options = {}) {
    ...
    try {
      ...
      if (parsed.type === 'nip05' || parsed.type === 'npub') {
        console.error('[NDK-NIP46] SECURITY: NIP-05 and npub connections blocked (no secret mechanism)');
        return { 
          success: false, 
          error: 'NIP-05 and npub connections are not supported for security reasons. Please use a bunker:// URL with a secret from your signer app, or scan the QR code.',
      ...
      if (parsed.type === 'bunker' && !parsed.secret) {
        console.error('[NDK-NIP46] SECURITY: Bunker URL has no secret - rejecting to prevent hijacking');
        return { 
          success: false, 
          error: 'This bunker URL does not contain a verification secret. For your security, please generate a NEW bunker URL from your signer app that includes a secret.',
          securityRejection: true,
          noSecret: true
        };
      }
  ...
  static async signEvent(eventTemplate) {
    ...
    try {
      ...
      console.log('[NDK-NIP46] v63: NDKEvent signed, id:', ndkEvent.id?.slice(0, 16) + '...');
      console.log('[NDK-NIP46] v63: NDKEvent pubkey:', ndkEvent.pubkey?.slice(0, 16) + '...');
      ...
      console.log('[NDK-NIP46] v63: Raw event pubkey:', signedEvent.pubkey?.slice(0, 16) + '...');
      console.log('[NDK-NIP46] v63: Raw event sig:', signedEvent.sig?.slice(0, 16) + '...');
