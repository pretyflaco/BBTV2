...
const formatTransactionType = (settlementVia, initiationVia) => {
...
export const TransactionReceiptDocument = ({ transaction, generatedAt, logoDataUrl }) => {
  const {
    id,
    direction,
    status,
  ...
  const paymentHash = initiationVia?.__typename === 'InitiationViaLn' 
    ? initiationVia.paymentHash 
    : null;
  
  const preimage = (settlementVia?.__typename === 'SettlementViaLn' || 
                    settlementVia?.__typename === 'SettlementViaIntraLedger')
    ? settlementVia.preImage
    : null;

  const onChainHash = settlementVia?.__typename === 'SettlementViaOnChain'
    ? settlementVia.transactionHash
    : null;

  const onChainAddress = initiationVia?.__typename === 'InitiationViaOnChain'
    ? initiationVia.address
    : null;

  // Get counterparty without @ prefix
  const counterparty = settlementVia?.__typename === 'SettlementViaIntraLedger'
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.amountSection}>
          ...
          {secondaryAmount && (
            <Text style={styles.amountSubtext}>{secondaryAmount}</Text>
          )}
        </View>
