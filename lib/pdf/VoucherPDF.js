...
const VoucherCard = ({ voucher, format, styles }) => {
  ...
  const isBtcDisplayCurrency = !voucher.fiatAmount || 
    (voucher.fiatAmount && (voucher.fiatAmount.includes('BTC') || voucher.fiatAmount.includes('sats')));
  
  // Format USD amount for display
  const formatUsdAmount = (cents) => {
    if (!cents) return null;
    return `$${(cents / 100).toFixed(2)} USD`;
  };
  
  // Get retention notice text based on voucher type
  const getRetentionNotice = () => {
    if (isUsdVoucher) {
        ...
        {isUsdVoucher && voucher.usdAmountCents ? (
          <>
            ...
            <View style={styles.infoRow}>
              ...
              <Text style={styles.infoValue}>{formatUsdAmount(voucher.usdAmountCents)}</Text>
            </View>
            ...
            ) : isBtcDisplayCurrency && voucher.commissionPercent > 0 ? (
              // Edge case: BTC display + USD voucher with commission - show original USD price
              <View style={styles.infoRow}>
                <Text style={styles.infoLabel}>Price:</Text>
                <Text style={styles.infoValue}>{formatUsdAmount(Math.round(voucher.usdAmountCents / (1 - voucher.commissionPercent / 100)))}</Text>
              </View>
            ) : null}
          </>
        ) : (
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.reissueLabel}>
          <Text style={styles.reissueLabelText}>{isUsdVoucher ? 'Reissued USD Voucher' : 'Reissued Voucher'}</Text>
        </View>
      </View>
      ...
      <View style={styles.infoSection}>
        ...
        <View style={[styles.infoRow, { marginTop: 8 }]}>
          <Text style={{ fontSize: 9, color: '#666666', fontStyle: 'italic' }}>{getRetentionNotice()}</Text>
        </View>
      </View>
