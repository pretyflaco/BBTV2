 ...
 * Split into:
 * - CITRUSRATE_EXCLUSIVE_CURRENCIES: 24 currencies NOT available in Blink API
 * - CITRUSRATE_ALT_CURRENCIES: 16 alternative rate sources for currencies also in Blink
 */
 ...
 */
const CITRUSRATE_EXCLUSIVE_CURRENCIES = [
  {
    id: 'AOA',
    name: 'Angolan Kwanza',
    flag: '🇦🇴',
 ...
 */
const CITRUSRATE_ALT_CURRENCIES = [
  {
    id: 'ETB_CITRUS',
    baseId: 'ETB',
    displayId: 'ETB (citrus)',
 ...
 */
const CITRUSRATE_EXCLUSIVE_IDS = CITRUSRATE_EXCLUSIVE_CURRENCIES.map(c => c.id);

/**
 * List of Citrusrate alternative currency IDs for quick lookup
 */
const CITRUSRATE_ALT_IDS = CITRUSRATE_ALT_CURRENCIES.map(c => c.id);
 ...
 * @returns {boolean}
 */
function isCitrusrateExclusiveCurrency(currencyId) {
 ...
 * @returns {boolean}
 */
function isCitrusrateAltCurrency(currencyId) {
  return currencyId.endsWith('_CITRUS');
}

/**
 * Get the base currency ID from a Citrusrate alt currency
 * @param {string} currencyId - e.g., 'NGN_CITRUS'
 * @returns {string} - e.g., 'NGN'
 */
function getCitrusrateBaseCurrency(currencyId) {
 ...
 * @returns {object|null}
 */
function getCitrusrateExclusiveCurrency(currencyId) {
 ...
 * @returns {object|null}
 */
function getCitrusrateAltCurrency(currencyId) {
  return CITRUSRATE_ALT_CURRENCIES.find(c => c.id === currencyId) || null;
}
...
module.exports = {
  ...
  getCitrusrateExclusiveCurrency,
  getCitrusrateAltCurrency
};
