...
class ProfileStorage {
  ...
  static getProfileByPublicKey(publicKey) {
    ...
    return profiles.find(p => p.publicKey === publicKey) || null;
  }
  ...
  static importProfiles(data, merge = true) {
    ...
    if (merge) {
      ...
      importedProfiles.forEach(imported => {
        const existingIndex = existing.findIndex(e => e.publicKey === imported.publicKey);
    ...
    if (data.activeProfileId) {
      ...
      const activeProfile = profiles.find(p => 
        p.publicKey === (data.profiles || []).find(dp => dp.id === data.activeProfileId)?.publicKey
      );
      if (activeProfile) {
