...
class HybridStore {
   ...
   */
  hashApiKey(apiKey, nwcActive = false) {
    if (!apiKey) {
  ...
  async storeTipData(paymentHash, tipData) {
    ...
    try {
      ...
      const userApiKeyHash = this.hashApiKey(userApiKey, nwcActive); // Uses placeholder for NWC-only users
      
      // For NWC-only users, use a placeholder wallet ID to satisfy NOT NULL constraint
      const effectiveWalletId = userWalletId || (nwcActive ? 'NWC_ONLY_USER' : 'UNKNOWN_USER');
