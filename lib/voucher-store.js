...
class VoucherStore {
   ...
   * @param {string} options.walletCurrency - Wallet currency type ('BTC' or 'USD')
   * @param {number} options.usdAmount - USD amount in cents (for USD vouchers only)
  ...
  async createVoucher(amount, apiKey, walletId, options = {}) {
    ...
    const usdAmountCents = options.usdAmount || null;
    ...
    try {
      const result = await query(
        ...
        [chargeId, amount, walletId, apiKeyEncrypted, now, expiresAt, expiryId,
         ...
         options.commissionPercent || 0, environment, walletCurrency, usdAmountCents]
      );
      ...
      const currencyInfo = walletCurrency === 'USD' ? ` [USD: $${(usdAmountCents / 100).toFixed(2)}]` : '';
      console.log(`[VoucherStore] Created voucher: ${chargeId} for ${amount} sats${currencyInfo}, expires: ${expiryDate}${options.commissionPercent ? ` (${options.commissionPercent}% commission)` : ''}${environment !== 'production' ? ` [${environment}]` : ''}`);
