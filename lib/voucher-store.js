...
class VoucherStore {
  ...
  async createVoucher(amount, apiKey, walletId, options = {}) {
    ...
    try {
      const result = await query(
        `INSERT INTO vouchers 
         (id, amount_sats, wallet_id, api_key_encrypted, status, claimed,
          created_at, expires_at, expiry_id, display_amount, display_currency, commission_percent)
         VALUES ($1, $2, $3, $4, 'ACTIVE', false, $5, $6, $7, $8, $9, $10)
         RETURNING *`,
      ...
      console.log(`[VoucherStore] Created voucher: ${chargeId} for ${amount} sats, expires: ${expiryDate}${options.commissionPercent ? ` (${options.commissionPercent}% commission)` : ''}`);
