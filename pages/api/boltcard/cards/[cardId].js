 ...
 */
async function handleUpdate(req, res, cardId) {
  try {
    const { name, maxTxAmount, dailyLimit } = req.body;
    ...
    if (maxTxAmount !== undefined) updates.maxTxAmount = maxTxAmount ? parseInt(maxTxAmount) : null;
    if (dailyLimit !== undefined) updates.dailyLimit = dailyLimit ? parseInt(dailyLimit) : null;
 ...
 */
async function handleAction(req, res, cardId) {
  try {
    ...
    switch (action) {
      ...
      case 'adjust':
        ...
        message = success ? `Balance adjusted by ${adjustAmount}` : 'Failed to adjust balance';
        break;
