...
export default async function handler(req, res) {
  ...
  try {
    ...
    const userPubkey = verification.pubkey;
    const { uid, version = 1 } = req.body;
    ...
    if (!issuerKey) {
      return res.status(404).json({ 
        ...
        message: 'You have not previously programmed any cards. Recovery is only possible for cards you have programmed.'
      });
    }
    ...
    if (existingCard) {
      ...
      if (existingCard.ownerPubkey === userPubkey) {
        return res.status(409).json({ 
          ...
          message: 'This card is still registered. Use the card details page to get wipe keys.',
    ...
    console.log(`[RecoverKeys] Generated recovery keys for UID ${normalizedUid} (key version ${keyVersion}) for user ${userPubkey.substring(0, 8)}...`);
