...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (card.ownerPubkey !== userPubkey) {
      console.warn(`[WipeKeys] Unauthorized access attempt: user ${userPubkey.substring(0, 8)}... tried to access card owned by ${card.ownerPubkey.substring(0, 8)}...`);
    ...
    if (!issuerKey) {
      console.error(`[WipeKeys] IssuerKey not found for owner: ${userPubkey.substring(0, 8)}...`);
    ...
    console.log(`[WipeKeys] Generated wipe keys for card ${cardId} (version ${card.version}) for user ${userPubkey.substring(0, 8)}...`);
