...
const { getProviderForCurrency, getBaseCurrency } = require('../../../lib/rate-providers/index');
...
export default async function handler(req, res) {
  ...
  try {
    const { currency, apiKey, useBlinkpos } = req.body;
    ...
    if (providerId === 'citrusrate_street') {
      ...
      try {
        ...
        return res.status(502).json({ 
          error: `Failed to fetch street rate for ${baseCurrency}: ${error.message}`,
          success: false,
      ...
      try {
        ...
        return res.status(502).json({ 
          error: `Failed to fetch Citrusrate rate for ${baseCurrency}: ${error.message}`,
          success: false,
