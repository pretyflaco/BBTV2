...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!prLower.startsWith('lnbc') && !prLower.startsWith('lntb') && !prLower.startsWith('lntbs')) {
    ...
    try {
      ...
      const isSuccess = paymentResult.status === 'SUCCESS' || 
                        paymentResult.status === 'ALREADY_PAID';
    ...
    const elapsed = Date.now() - startTime;
    console.error(`❌ Voucher callback error after ${elapsed}ms:`, error.message);
    ...
    } else if (error.message.includes('already paid') || error.message.includes('ALREADY_PAID')) {
