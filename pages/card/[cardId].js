...
export default function CardholderPage() {
  ...
  useEffect(() => {
    if (!cardId || !piccData || !sunMac) {
      ...
      if (router.isReady && (!piccData || !sunMac)) {
        setError('Missing authentication parameters. Please tap your card to check balance.');
        setLoading(false);
      }
      return;
    }
    ...
    const fetchBalance = async () => {
      ...
      try {
        ...
        setError(err.message || 'Failed to load card balance');
      } finally {
        setLoading(false);
      }
    };
  ...
  const pageTitle = data?.card?.name 
    ? `${data.card.name} - Balance`
    : 'Boltcard Balance';
