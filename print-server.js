    ...
    req.on("end", async () => {
      try {
        ...
        if (!json.data) {
          ...
          res.end(
            JSON.stringify({
              error: 'Missing "data" field (Base64 encoded ESC/POS bytes)',
            }),
          )
          return
        }
...
server.listen(PORT, () => {
  ...
  console.log(
    `║  Status:    ${(printerStatus.connected ? "Connected" : "Not connected").padEnd(46)}║`,
  )
...
setInterval(() => {
  ...
  if (wasConnected !== printerStatus.connected) {
    console.log(
      `[STATUS] Printer ${printerStatus.connected ? "connected" : "disconnected"}`,
    )
