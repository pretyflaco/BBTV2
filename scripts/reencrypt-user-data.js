...
// Configuration
const OLD_KEY = process.env.OLD_KEY || process.env.OLD_ENCRYPTION_KEY || 'blink-encryption-key-2025';
const NEW_KEY = process.env.NEW_KEY || process.env.ENCRYPTION_KEY || 'bbtv2_production_encryption_key_2025_strong_random_string';
  ...
  if (data.voucherWallet?.apiKey) {
    ...
    if (decrypted) {
      ...
      if (verifyDecrypt === decrypted) {
        ...
        stats.errors.push({ file: fileName, error: 'voucherWallet: Re-encryption verification failed' });
      }
    } else {
      // Try decrypting with new key (already migrated?)
      const alreadyNew = decrypt(encryptedKey, NEW_KEY);
      if (alreadyNew) {
        stats.skipped.push({ file: fileName, field: 'voucherWallet', reason: 'Already using new key' });
      } else {
        stats.errors.push({ file: fileName, error: 'voucherWallet: Cannot decrypt with either key' });
      }
    }
  }
    ...
    data.blinkApiAccounts.forEach((account, idx) => {
      if (account.apiKey) {
        ...
        if (decrypted) {
          ...
          if (verifyDecrypt === decrypted) {
            changes.push(`blinkApiAccounts[${idx}].apiKey (${account.label || 'unnamed'})`);
            data.blinkApiAccounts[idx].apiKey = newEncrypted;
            stats.blinkApiAccountsFixed++;
            modified = true;
          } else {
            stats.errors.push({ file: fileName, error: `blinkApiAccounts[${idx}]: Re-encryption verification failed` });
          }
        } else {
          const alreadyNew = decrypt(account.apiKey, NEW_KEY);
          if (alreadyNew) {
            stats.skipped.push({ file: fileName, field: `blinkApiAccounts[${idx}]`, reason: 'Already using new key' });
          } else {
            stats.errors.push({ file: fileName, error: `blinkApiAccounts[${idx}]: Cannot decrypt with either key` });
          }
        }
      }
    });
      ...
      if (conn.uri) {
        ...
        if (decrypted) {
          ...
          if (verifyDecrypt === decrypted) {
            ...
            stats.errors.push({ file: fileName, error: `nwcConnections[${idx}]: Re-encryption verification failed` });
          }
        } else {
          const alreadyNew = decrypt(conn.uri, NEW_KEY);
          if (alreadyNew) {
            stats.skipped.push({ file: fileName, field: `nwcConnections[${idx}]`, reason: 'Already using new key' });
          } else {
            stats.errors.push({ file: fileName, error: `nwcConnections[${idx}]: Cannot decrypt with either key` });
          }
        }
      }
    });
