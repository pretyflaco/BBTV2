...
describe("useSplitProfiles", () => {
  ...
  const createMockRecipient = (
    overrides: Partial<SplitRecipient> = {},
  ): SplitRecipient => ({
    username: "alice",
    validated: true,
    type: "blink",
  ...
  describe("updateRecipientWeight", () => {
    it("updates the weight of a specific recipient", () => {
      ...
      act(() => {
        result.current.addRecipientToList(
          createMockRecipient({ username: "alice", weight: 50 }),
        )
        result.current.addRecipientToList(
          createMockRecipient({ username: "bob", weight: 50 }),
      ...
      const alice = result.current.newSplitProfileRecipients.find(
        (r) => r.username === "alice",
      )
      const bob = result.current.newSplitProfileRecipients.find(
        (r) => r.username === "bob",
      )
  ...
  describe("toggleRecipientLock", () => {
    it("toggles the locked state of a recipient", () => {
      ...
      act(() => {
        result.current.addRecipientToList(
          createMockRecipient({ username: "alice", locked: false }),
        )
      })
  ...
  describe("typical workflow scenarios", () => {
    it("handles create new profile workflow", () => {
      ...
      act(() => {
        result.current.addRecipientToList(
          createMockRecipient({ username: "alice", weight: 100 }),
        )
      })
